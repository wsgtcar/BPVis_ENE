...
def _auth_credentials_status(email: str, password: str) -> Tuple[
    bool, str, Optional[str], Optional[str], Optional[str]]:
...
def _render_login_page() -> None:
    ...
    with middle:
        ...
        try:
            ...
            st.error(
                f"Could not read `{AUTH_USERS_FILENAME}`. Required columns: `email`, `password`, `expiration_date`, `access_mode`, `data_source_file`.")
            st.caption(str(exc))
            st.stop()
...
def _apply_master_energy_use_filter_to_energy_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not selected:
        return df
    keep = (["Month"] if "Month" in df.columns else []) + [c for c in energy_cols if
                                                           _canon_enduse_name(c) in set(selected)]
...
def get_energy_balance_df(
    ...
    if use_scenario_override:
        sc_name = str(scenario_name) if scenario_name is not None and str(
            scenario_name).strip() else _active_scenario_name()
    ...
    for sc_name, df in overrides.items():
        ...
        for _, r in long_df.iterrows():
            rows.append({
                ...
                "kWh": _to_float_lcc(r.get("kWh"), 0.0) if "_to_float_lcc" in globals() else pd.to_numeric(r.get("kWh"),
                                                                                                           errors="coerce"),
            })
...
def _tariff_apply_rates_to_rows(
    ...
    eq_map = {src: float((tariff_details.get(src, {}) or {}).get("equivalent_tariff", 0.0)) for src in
              ENERGY_SOURCE_ORDER}
    base_map = {src: float((tariff_details.get(src, {}) or {}).get("tariff_per_kwh", 0.0)) for src in
                ENERGY_SOURCE_ORDER}
...
def parse_color_settings_df(df: Optional[pd.DataFrame]) -> Tuple[
...
def _canon_scenario_payload(payload: dict) -> dict:
            ...
            for rec in investments:
                if isinstance(rec, dict):
                    if "Assigned End Uses" in rec:
                        ...
                        rec["Assigned End Uses"] = ", ".join(
                            [_canon_enduse_name(str(x).strip()) for x in vals if str(x).strip()])
    ...
    if isinstance(lcc_global, dict):
        ...
        if isinstance(selected, list):
            lcc_global["selected_operational_end_uses"] = [_canon_enduse_name(str(x)) for x in selected if
                                                           str(x).strip()]
...
def _normalize_lcc_global_payload(lcc_global_payload, end_uses: list) -> dict:
    ...
    out["capex_inflation_pct"] = _to_float_lcc(lcc_global_payload.get("capex_inflation_pct"),
                                               defaults["capex_inflation_pct"])
...
def _capture_lcc_global_from_draft_widgets(end_uses: list) -> dict:
    ...
    defaults = _default_lcc_global_payload(end_uses)
    selected = st.session_state.get(_lcc_global_draft_key("selected_operational_end_uses"),
                                    defaults["selected_operational_end_uses"])
    if not isinstance(selected, list):
        selected = defaults["selected_operational_end_uses"]

    payload = {
        "analysis_period": max(1, _to_int_lcc(
            st.session_state.get(_lcc_global_draft_key("analysis_period"), defaults["analysis_period"]),
            defaults["analysis_period"])),
        "interest_rate_pct": _to_float_lcc(
            st.session_state.get(_lcc_global_draft_key("interest_rate_pct"), defaults["interest_rate_pct"]),
            defaults["interest_rate_pct"]),
        "capex_inflation_pct": _to_float_lcc(
            st.session_state.get(_lcc_global_draft_key("capex_inflation_pct"), defaults["capex_inflation_pct"]),
            defaults["capex_inflation_pct"]),
        "energy_inflation_pct": {
            src: _to_float_lcc(
                st.session_state.get(_lcc_energy_inflation_draft_key(src),
        ...
        "payback_reference_scenario": str(
            st.session_state.get(_lcc_global_draft_key("payback_reference_scenario"), "") or ""),
    }
            ...
            if not isinstance(saved, dict):
                for payload_i in scenarios.values():
                    if isinstance(payload_i, dict) and isinstance(payload_i.get("lcc"), dict):
                        ...
                        if any(k in lcc_i for k in
                               ["analysis_period", "selected_operational_end_uses", "energy_inflation_pct"]):
                            saved = lcc_i
                            break

    payload = _normalize_lcc_global_payload(saved, end_uses) if isinstance(saved,
                                                                           dict) else _default_lcc_global_payload(
        end_uses)
...
def _ensure_lcc_global_state(end_uses: list, scenarios: Optional[dict] = None,
                             active_payload: Optional[dict] = None) -> None:
...
def parse_lcc_global_df(df: Optional[pd.DataFrame], end_uses: list) -> Optional[dict]:
    ...
    payload = {
        ...
        "payback_reference_scenario": str(
            kv.get("Payback_Reference_Scenario", kv.get("Payback Reference Scenario", "")) or ""),
    }
    return _normalize_lcc_global_payload(payload, end_uses)
...
def merge_lcc_sheets_into_scenarios(
    ...
    for sc_name, payload in list(scenarios.items()):
        ...
        if global_payload is not None:
            ...
            payload["lcc_global"] = _normalize_lcc_global_payload(payload.get("lcc_global", payload.get("lcc", {})),
                                                                  end_uses)
...
def compute_lcc_cashflow_table(
    ...
    for _, r in inv_df.iterrows():
        ...
        def _append_allocated_cost(year: int, cost_type: str, nominal_total: float) -> None:
            ...
            discounted_total = nominal_total / ((1.0 + discount_rate) ** offset_local) if (
                                                                                                      1.0 + discount_rate) != 0 else nominal_total
...
def discounted_payback_period(active_cf: pd.DataFrame, reference_cf: pd.DataFrame, project_year: int) -> Optional[
    float]:
...
def _report_stacked_monthly_chart(df: pd.DataFrame, value_col: str, category_col: str, title: str, y_label: str,
                                  color_dict: dict) -> io.BytesIO:
    import matplotlib.pyplot as plt
    ...
    cats = _report_unique_order([c for c in END_USE_ORDER + ENERGY_SOURCE_ORDER + sorted(
        work[category_col].dropna().astype(str).unique().tolist()) if c in set(work[category_col].astype(str))])
    pivot = work.groupby(["_report_x", category_col], observed=False)[value_col].sum().unstack(fill_value=0.0).reindex(
        x_order).fillna(0.0)
    ...
    ax.plot(pivot.index.astype(str), totals, color="black", linestyle="--", linewidth=REPORT_LINE_WIDTH, marker="o",
            markersize=REPORT_MARKER_SIZE, label="Net total")
...
def _report_annual_stacked_chart(df: pd.DataFrame, value_col: str, category_col: str, title: str, y_label: str,
                                 color_dict: dict) -> io.BytesIO:
    import matplotlib.pyplot as plt
    ...
    cats = _report_unique_order(
        [c for c in END_USE_ORDER + ENERGY_SOURCE_ORDER + sorted(totals.index.astype(str).tolist()) if
         c in set(totals.index.astype(str))])
    pos_bottom = 0.0
    neg_bottom = 0.0
    for cat, col in zip(cats, _report_colors_for(cats, color_dict)):
...
def _report_line_chart(series_dict: Dict[str, pd.Series], title: str, y_label: str, color_map_in: Optional[dict] = None,
                       dashed: Optional[set] = None) -> io.BytesIO:
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(figsize=(7.2, 3.75))
    ...
    for i, (name, ser) in enumerate(series_dict.items()):
        ...
        ax.plot(ser.index.astype(int), ser.values.astype(float), label=name, color=col, linewidth=REPORT_LINE_WIDTH,
                linestyle="--" if name in dashed else "-", marker="o", markersize=REPORT_MARKER_SIZE)
    ax.set_title(title, fontsize=REPORT_CHART_TITLE_SIZE, weight="bold")
...
def _report_bar_chart(df: pd.DataFrame, x: str, y: str, title: str, y_label: str,
                      color: Optional[str] = None) -> io.BytesIO:
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(figsize=(7.2, 3.75))
...
def _report_load_duration(df_loads: pd.DataFrame, load_col: str, title: str) -> io.BytesIO:
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(figsize=(7.2, 3.75))
    vals = pd.to_numeric(df_loads.get(load_col, pd.Series(dtype=float)), errors="coerce").dropna().sort_values(
...
def _report_crrem_limits_for_context(crrem: dict, target_id: str, crrem_use: str, mixed_records, years: list) -> Tuple[
    pd.Series, pd.Series]:
    ...
    try:
        ...
        if str(crrem_use) != "Mixed Use":
            ...
            return carbon_pivot[p_code].reindex(years_avail).astype(float), eui_pivot[p_code].reindex(
                years_avail).astype(float)
...
def _report_eui_series_for_payload(df_energy: pd.DataFrame, payload: dict, years: list,
                                   project_area: float) -> pd.Series:
    ...
    try:
        ...
        onsite = set(get_onsite_generation_enduses(rows["End_Use"].unique())) | {ONSITE_GENERATION_ENDUSE,
                                                                                 LEGACY_PV_ENDUSE}
...
def _scenario_comparison_lcc_global_payload(base_lcc_global: dict, end_uses: list,
                                            apply_lcc_filter: bool = True) -> dict:
...
def _scenario_radar_kpi_order(period_years: Optional[int] = None, radar_raw_df: Optional[pd.DataFrame] = None) -> list:
    ...
    if isinstance(radar_raw_df, pd.DataFrame) and not radar_raw_df.empty and "KPI" in radar_raw_df.columns:
        ...
        lcc_label = lcc_labels[0] if lcc_labels else _scenario_lcc_kpi_label(
            period_years or _get_scenario_comparison_analysis_period())
        emis_label = emis_labels[0] if emis_labels else _scenario_lc_emissions_kpi_label(
            period_years or _get_scenario_comparison_analysis_period())
    else:
    ...
    for sc_name in scenario_order:
        ...
        if include_capex:
            try:
                ...
                capex_total_sc = pd.to_numeric(inv_df_sc.get("Investment Cost", pd.Series(dtype=float)),
        ...
        try:
            ...
            df_loads_sc_50 = get_loads_balance_df(file_bytes, filename, scenario_name=sc_name_str,
        ...
        rows.extend([
            ...
            {"Scenario": sc_name_str, "KPI": "Annual Energy Cost /m²", "Value": annual_energy_cost_m2,
        ...
        if include_capex:
            rows.append(
                {"Scenario": sc_name_str, "KPI": "Capex /m²", "Value": capex_m2, "Unit": f"{currency_symbol_in}/m²"})
        if include_annual_opex:
            rows.append({"Scenario": sc_name_str, "KPI": "Annual OPEX /m²", "Value": annual_opex_m2,
                         "Unit": f"{currency_symbol_in}/m²·a"})
...
def _prepare_scenario_radar_plot_dfs(
    ...
    if not improvement_df.empty:
        vals = pd.to_numeric(improvement_df.get("Improvement vs Reference %"), errors="coerce").replace(
        ...
        improvement_df["Radial"] = pd.to_numeric(improvement_df["Improvement vs Reference %"],
                                                 errors="coerce") - axis_min
...
def _scenario_kpi_scatter_plotly_figure(
        ...
        title: str = "Scenario KPI bar summary",
        height: int = 620,
) -> go.Figure:
    """Create one scenario KPI bar chart with one independent y-axis per KPI.
    ...
    Bars are used instead of scatter markers to improve readability.
    """
    if radar_raw_df is None or radar_raw_df.empty:
        return go.Figure()
    ...
    group_width = 0.72
    bar_width = min(0.18, group_width / max(n_sc, 1) * 0.82)
    if n_sc == 1:
        offsets = [0.0]
    else:
        # centered offsets within each KPI group
        offsets = np.linspace(-group_width / 2.0 + bar_width / 2.0, group_width / 2.0 - bar_width / 2.0, n_sc)
    ...
    for kpi_i, kpi in enumerate(kpis):
        ...
        for sc_i, sc in enumerate(scenario_order):
            ...
            fig.add_trace(go.Bar(
                x=[float(kpi_i) + float(offsets[min(sc_i, len(offsets) - 1)])],
                y=[float(val)],
...
def _prepare_scenario_delta_analysis_dfs(
    ...
    for sc in scenario_order_raw:
        ...
        for kpi in kpi_order:
            ...
            long_rows.append({
                ...
                "Unit": unit,
                "Status": "Better / lower" if improvement > 1e-9 else (
                    "Worse / higher" if improvement < -1e-9 else "No change"),
            })
    ...
    for sc in delta_long["Scenario"].dropna().astype(str).drop_duplicates().tolist():
        ...
        lcc_kpi_label = next((k for k in kpi_order if _is_scenario_lcc_kpi(k)),
                             _scenario_lcc_kpi_label(_get_scenario_comparison_analysis_period()))
        lc_em_kpi_label = next((k for k in kpi_order if _is_scenario_lc_emissions_kpi(k)),
                               _scenario_lc_emissions_kpi_label(_get_scenario_comparison_analysis_period()))
        ...
        summary_rows.append({
            ...
            f"LCC {str(lcc_kpi_label).split(' ')[1] if len(str(lcc_kpi_label).split(' ')) > 1 else ''}y improvement (%)": _val(
                lcc_kpi_label, "Improvement vs Reference (%)"),
            "Annual Emissions improvement (%)": _val("Annual Emissions /m²", "Improvement vs Reference (%)"),
            f"LC Emissions {str(lc_em_kpi_label).split(' ')[2] if len(str(lc_em_kpi_label).split(' ')) > 2 else ''}y improvement (%)": _val(
                lc_em_kpi_label, "Improvement vs Reference (%)"),
...
def _scenario_delta_improvement_plotly_figure(
    ...
    dfp["Scenario"] = dfp["Scenario"].astype(str)
    dfp["KPI"] = dfp["KPI"].astype(str)
    dfp["Improvement vs Reference (%)"] = pd.to_numeric(dfp["Improvement vs Reference (%)"], errors="coerce").replace(
        [np.inf, -np.inf], np.nan)
...
def _scenario_delta_mac_plotly_figure(
    ...
    colors = [(scenario_color_map_in or {}).get(str(sc), SCENARIO_COLOR_PALETTE[i % len(SCENARIO_COLOR_PALETTE)]) for
              i, sc in enumerate(dfp["Scenario"].astype(str))]
...
def _scenario_radar_plotly_figure(
    ...
    if mode == "improvement":
        ...
        # Keep 5% radial headroom outside the 100% axis maximum. Values remain
...
def _report_radar_chart(
    ...
    labels = [k for k in _scenario_radar_kpi_order(radar_raw_df=plot_df) if
              plot_df is not None and not plot_df.loc[plot_df["KPI"].astype(str) == k].empty]
    if not labels:
    ...
    for i, sc in enumerate([str(s) for s in scenario_order_in]):
        ...
        plotted.append(sc)
    if plotted:
        ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.22), ncol=min(3, max(1, len(plotted))), fontsize=6.6,
                  frameon=False)
...
def _report_scenario_kpi_scatter_chart(
    ...
    """Create one KPI bar chart with separate y-axis scales for the PDF report."""
    import matplotlib.pyplot as plt
    import numpy as _np
    ...
    bar_width = min(0.18, group_width / max(n_sc, 1) * 0.82)
    if n_sc == 1:
        offsets = _np.array([0.0])
    else:
        offsets = _np.linspace(-group_width / 2.0 + bar_width / 2.0, group_width / 2.0 - bar_width / 2.0, n_sc)
    ...
    for kpi_i, kpi in enumerate(kpis):
        ...
        for sc_i, sc in enumerate(scenario_order):
            ...
            if hit.empty or pd.isna(hit.iloc[0]):
                continue
            col = (scenario_color_map_in or {}).get(sc, SCENARIO_COLOR_PALETTE[sc_i % len(SCENARIO_COLOR_PALETTE)])
            handle = ax.bar(kpi_i + float(offsets[min(sc_i, len(offsets) - 1)]), float(hit.iloc[0]),
                            width=float(bar_width), color=col, edgecolor="white", linewidth=0.45, zorder=4)
            if kpi_i == 0:
    ...
    if legend_handles:
        host.legend(legend_handles, legend_labels, loc="lower center", bbox_to_anchor=(0.5, -0.34),
                    ncol=min(3, max(1, len(legend_labels))), fontsize=6.5, frameon=False)
...
def _report_scenario_delta_improvement_chart(
    ...
    offsets = _np.array([0.0]) if n_sc == 1 else _np.linspace(-group_width / 2 + bar_width / 2,
                                                              group_width / 2 - bar_width / 2, n_sc)
    for i, sc in enumerate(scenario_order):
        vals = []
        for kpi in kpis:
            hit = dfp.loc[
                (dfp["Scenario"].astype(str) == sc) & (dfp["KPI"].astype(str) == kpi), "Improvement vs Reference (%)"]
            vals.append(float(hit.iloc[0]) if not hit.empty and pd.notna(hit.iloc[0]) else _np.nan)
        col = (scenario_color_map_in or {}).get(sc, SCENARIO_COLOR_PALETTE[i % len(SCENARIO_COLOR_PALETTE)])
        bars = ax.bar(x + offsets[min(i, len(offsets) - 1)], vals, width=bar_width, label=sc, color=col,
                      edgecolor="white", linewidth=0.4)
        for rect, v in zip(bars, vals):
            if _np.isfinite(v):
                ax.text(rect.get_x() + rect.get_width() / 2, v, f"{v:,.0f}%", ha="center",
                        va="bottom" if v >= 0 else "top", fontsize=5.7, rotation=90)
    ...
    _report_apply_axis_style(ax)
    ax.legend(loc="lower center", bbox_to_anchor=(0.5, -0.32), ncol=min(3, max(1, len(scenario_order))), fontsize=6.5,
              frameon=False)
...
def _report_scenario_delta_mac_chart(
    ...
    dfp = dfp.sort_values("Scenario")
    colors_l = [(scenario_color_map_in or {}).get(str(sc), SCENARIO_COLOR_PALETTE[i % len(SCENARIO_COLOR_PALETTE)]) for
                i, sc in enumerate(dfp["Scenario"].astype(str))]
    fig, ax = plt.subplots(figsize=(7.2, 3.4), dpi=180)
    bars = ax.bar(dfp["Scenario"].astype(str), dfp[mac_col].astype(float), color=colors_l, edgecolor="white",
                  linewidth=0.5)
    for rect, v in zip(bars, dfp[mac_col].astype(float)):
        ax.text(rect.get_x() + rect.get_width() / 2, v, f"{v:,.0f}", ha="center", va="bottom" if v >= 0 else "top",
                fontsize=6.2)
    ax.axhline(0, color="black", linestyle="--", linewidth=0.8)
...
def generate_bpvis_pdf_report(file_bytes: bytes, filename: str = "") -> bytes:
    ...
    project_name_r = str(
        st.session_state.get("project_name", "Building Performance Dashboard") or "Building Performance Dashboard")
    ...
    colors_scenarios = st.session_state.get("color_map_scenarios", default_scenario_color_map(
        list(st.session_state.get("scenarios", {}).keys())))
    ...
    styles.add(
        ParagraphStyle(name="ReportTitle", parent=styles["Title"], fontName="Helvetica-Bold", fontSize=22, leading=26,
                       alignment=TA_CENTER, spaceAfter=12))
    styles.add(ParagraphStyle(name="SectionTitle", parent=styles["Heading1"], fontName="Helvetica-Bold", fontSize=17,
                              leading=21, spaceAfter=10))
    styles.add(
        ParagraphStyle(name="CaptionSmall", parent=styles["BodyText"], fontName="Helvetica", fontSize=7.4, leading=9,
    ...
    story.append(Paragraph(
        "This report presents the current active scenario only. It excludes the Raw Data tab and is formatted for A4 output. Each section starts on a new page.",
        styles["BodyText"]))
    ...
    _report_add_chart(story, styles, "Monthly energy by end use",
                      _report_stacked_monthly_chart(rows_raw, "kWh", "End_Use", "Monthly Energy by End Use",
                                                    "kWh/month", colors_eu),
                      "Monthly values are raw Energy_Balance values by end use. The dashed line is the monthly net total.")
    _report_add_chart(story, styles, "Annual energy by end use",
                      _report_annual_stacked_chart(rows_raw, "kWh", "End_Use", "Annual Energy by End Use", "kWh/a",
                                                   colors_eu),
                      "Annual values are the sum of all monthly Energy_Balance values. Negative on-site generation is shown below zero.")
    pie_eu = (totals_eu[totals_eu > 0] / project_area_r) if project_area_r else totals_eu[totals_eu > 0]
    _report_add_chart(story, styles, "Energy use intensity by end use",
                      _report_pie_chart(pie_eu, "EUI Share by End Use", f"{eui_gross:,.1f}\nkWh/m²·a", colors_eu),
                      "The donut uses positive consumption only and divides annual kWh by project area.")
    _report_add_chart(story, styles, "Monthly energy by energy source", _report_stacked_monthly_chart(
        rows_raw.groupby(["Month", "Energy_Source"], as_index=False)["kWh"].sum(), "kWh", "Energy_Source",
        "Monthly Energy by Energy Source", "kWh/month", colors_src),
                      "Monthly values are grouped by the active scenario end-use to energy-source mapping.")
    _report_add_chart(story, styles, "Annual energy by energy source",
                      _report_annual_stacked_chart(rows_raw, "kWh", "Energy_Source", "Annual Energy by Energy Source",
                                                   "kWh/a", colors_src),
                      "Annual source totals are calculated using the active scenario energy-source mapping.")
    pie_src = (totals_src[totals_src > 0] / project_area_r) if project_area_r else totals_src[totals_src > 0]
    _report_add_chart(story, styles, "Energy use intensity by energy source",
                      _report_pie_chart(pie_src, "EUI Share by Energy Source", f"{pie_src.sum():,.1f}\nkWh/m²·a",
                                        colors_src),
                      "Source intensity is annual kWh per source divided by project area.")
    add_input_table("Relevant inputs", [
        ("Scenario", active_name),
        ("Project area", f"{project_area_r:,.1f} m²"),
        ("Energy-source mapping",
         ", ".join([f"{ui_name(k)}={v}" for k, v in (payload.get("mapping", {}) or {}).items()])),
    ])
    ...
    _report_add_chart(story, styles, "Monthly factored energy by end use",
                      _report_stacked_monthly_chart(rows_eff, "kWh", "End_Use",
                                                    "Monthly Energy by End Use - with Factors", "kWh/month", colors_eu),
                      "Each end-use kWh is divided by its efficiency factor before aggregation.")
    _report_add_chart(story, styles, "Annual factored energy by end use",
                      _report_annual_stacked_chart(rows_eff, "kWh", "End_Use",
                                                   "Annual Energy by End Use - with Factors", "kWh/a", colors_eu),
                      "Annual values are factored kWh after applying active scenario efficiency factors.")
    pie_eff_eu = (totals_eff_eu[totals_eff_eu > 0] / project_area_r) if project_area_r else totals_eff_eu[
        totals_eff_eu > 0]
    _report_add_chart(story, styles, "Factored EUI by end use",
                      _report_pie_chart(pie_eff_eu, "Factored EUI Share by End Use", f"{eui_eff_gross:,.1f}\nkWh/m²·a",
                                        colors_eu),
                      "The donut is based on positive factored consumption divided by project area.")
    _report_add_chart(story, styles, "Monthly factored energy by source", _report_stacked_monthly_chart(
        rows_eff.groupby(["Month", "Energy_Source"], as_index=False)["kWh"].sum(), "kWh", "Energy_Source",
        "Monthly Energy by Source - with Factors", "kWh/month", colors_src),
                      "Factored monthly kWh are grouped by energy source.")
    _report_add_chart(story, styles, "Annual factored energy by source",
                      _report_annual_stacked_chart(rows_eff, "kWh", "Energy_Source",
                                                   "Annual Energy by Source - with Factors", "kWh/a", colors_src),
                      "Source totals are after efficiency factors and source mapping.")
    pie_eff_src = (totals_eff_src[totals_eff_src > 0] / project_area_r) if project_area_r else totals_eff_src[
        totals_eff_src > 0]
    _report_add_chart(story, styles, "Factored EUI by energy source",
                      _report_pie_chart(pie_eff_src, "Factored EUI Share by Energy Source",
                                        f"{pie_eff_src.sum():,.1f}\nkWh/m²·a", colors_src),
                      "Source EUI is annual factored kWh divided by project area.")
    add_input_table("Relevant inputs", [(f"Efficiency - {ui_name(k)}", f"{float(v):,.4f}") for k, v in
                                        (payload.get("efficiency", {}) or {}).items()])
    ...
    _report_add_chart(story, styles, "Monthly CO₂ by end use",
                      _report_stacked_monthly_chart(rows_co2, "kgCO2", "End_Use", "Monthly CO₂ by End Use",
                                                    "kgCO₂/month", colors_eu),
                      "CO₂ is calculated as factored kWh multiplied by the mapped energy-source emission factor.")
    _report_add_chart(story, styles, "Annual CO₂ by end use",
                      _report_annual_stacked_chart(rows_co2, "kgCO2", "End_Use", "Annual CO₂ by End Use", "kgCO₂/a",
                                                   colors_eu),
                      "Annual CO₂ is the sum of all monthly emissions by end use.")
    pie_co2_eu = (totals_co2_eu[totals_co2_eu > 0] / project_area_r) if project_area_r else totals_co2_eu[
        totals_co2_eu > 0]
    _report_add_chart(story, styles, "CO₂ intensity by end use",
                      _report_pie_chart(pie_co2_eu, "CO₂ Intensity Share by End Use", f"{gross_co2_int:,.1f}\nkg/m²·a",
                                        colors_eu), "The donut uses positive annual kgCO₂ divided by project area.")
    _report_add_chart(story, styles, "Monthly CO₂ by energy source", _report_stacked_monthly_chart(
        rows_co2.groupby(["Month", "Energy_Source"], as_index=False)["kgCO2"].sum(), "kgCO2", "Energy_Source",
        "Monthly CO₂ by Energy Source", "kgCO₂/month", colors_src), "Monthly CO₂ is grouped by energy source.")
    _report_add_chart(story, styles, "Annual CO₂ by energy source",
                      _report_annual_stacked_chart(rows_co2, "kgCO2", "Energy_Source", "Annual CO₂ by Energy Source",
                                                   "kgCO₂/a", colors_src),
                      "Annual source emissions are factored kWh times source-specific emission factors.")
    pie_co2_src = (totals_co2_src[totals_co2_src > 0] / project_area_r) if project_area_r else totals_co2_src[
        totals_co2_src > 0]
    _report_add_chart(story, styles, "CO₂ intensity by energy source",
                      _report_pie_chart(pie_co2_src, "CO₂ Intensity Share by Energy Source",
                                        f"{pie_co2_src.sum():,.1f}\nkg/m²·a", colors_src),
                      "Source CO₂ intensity is annual kgCO₂ per source divided by project area.")
    add_input_table("Relevant inputs",
                    [(f"Emission factor - {src}", f"{factor_map.get(src, 0.0):,.5f} kgCO₂/kWh") for src in
                     ENERGY_SOURCE_ORDER])
    ...
    _report_add_chart(story, styles, "Monthly cost by end use",
                      _report_stacked_monthly_chart(rows_cost, "cost", "End_Use", "Monthly Cost by End Use",
                                                    f"{currency_r}/month", colors_eu),
                      "Cost is factored kWh multiplied by the active scenario tariff of the mapped energy source.")
    _report_add_chart(story, styles, "Annual cost by end use",
                      _report_annual_stacked_chart(rows_cost, "cost", "End_Use", "Annual Cost by End Use",
                                                   f"{currency_r}/a", colors_eu),
                      "Annual costs are the sum of monthly costs. Negative on-site generation appears as avoided cost if present.")
    pie_cost_eu = (totals_cost_eu[totals_cost_eu > 0] / project_area_r) if project_area_r else totals_cost_eu[
        totals_cost_eu > 0]
    _report_add_chart(story, styles, "Cost intensity by end use",
                      _report_pie_chart(pie_cost_eu, "Cost Intensity Share by End Use",
                                        f"{currency_r} {gross_cost_int:,.2f}\n/m²·a", colors_eu),
                      "The donut uses positive annual costs divided by project area.")
    _report_add_chart(story, styles, "Monthly cost by energy source", _report_stacked_monthly_chart(
        rows_cost.groupby(["Month", "Energy_Source"], as_index=False)["cost"].sum(), "cost", "Energy_Source",
        "Monthly Cost by Energy Source", f"{currency_r}/month", colors_src),
                      "Monthly energy costs are grouped by source.")
    _report_add_chart(story, styles, "Annual cost by energy source",
                      _report_annual_stacked_chart(rows_cost, "cost", "Energy_Source", "Annual Cost by Energy Source",
                                                   f"{currency_r}/a", colors_src),
                      "Annual source costs are factored kWh times source-specific tariffs.")
    pie_cost_src = (totals_cost_src[totals_cost_src > 0] / project_area_r) if project_area_r else totals_cost_src[
        totals_cost_src > 0]
    _report_add_chart(story, styles, "Cost intensity by energy source",
                      _report_pie_chart(pie_cost_src, "Cost Intensity Share by Energy Source",
                                        f"{currency_r} {pie_cost_src.sum():,.2f}\n/m²·a", colors_src),
                      "Source cost intensity is annual cost per source divided by project area.")
    ...
    load_cols = [c for c in load_cols if
                 pd.to_numeric(df_loads.get(c, pd.Series(dtype=float)), errors="coerce").notna().any()]
    ...
    if load_cols:
        ...
        for idx_load, selected_load in enumerate(load_cols, start=1):
            ...
            add_kpi_table(f"Load KPIs - {ui_name(selected_load)}", [
                ...
                ("95th percentile specific load",
                 f"{np.percentile(specific.dropna(), 95):,.1f} W/m²" if not specific.dropna().empty else "n/a"),
                ("80th percentile specific load",
                 f"{np.percentile(specific.dropna(), 80):,.1f} W/m²" if not specific.dropna().empty else "n/a"),
            ])
            if "month" in df_loads.columns:
                monthly_load = \
                df_loads.assign(_load=pd.to_numeric(df_loads[selected_load], errors="coerce")).groupby("month",
                                                                                                       as_index=False)[
                    "_load"].sum()
                _report_add_chart(story, styles, "Monthly load sum", _report_bar_chart(monthly_load, "month", "_load",
                                                                                       f"Monthly Load Sum - {ui_name(selected_load)}",
                                                                                       "kWh",
                                                                                       colors_loads.get(selected_load,
                                                                                                        CRREM_COLOR_BASELINE)),
                                  "Monthly load sum is calculated by summing the hourly load profile by month.")
            _report_add_chart(story, styles, "Hourly load heatmap", _report_heatmap(df_loads, selected_load,
                                                                                    f"Hourly Load Heatmap - {ui_name(selected_load)}"),
                              "The heatmap shows hourly load intensity by day of year and hour.")
            _report_add_chart(story, styles, "Load duration curve", _report_load_duration(df_loads, selected_load,
                                                                                          f"Load Duration Curve - {ui_name(selected_load)}"),
                              "The load duration curve sorts hourly load values descending and plots load against percentage of annual hours.")
            try:
                peaks = df_loads.loc[:,
                        [c for c in ["month", "day", "weekday", "hour", selected_load] if c in df_loads.columns]].copy()
                ...
                rows_peak = [["month", "day", "weekday", "hour", selected_load]] + peaks.fillna("").astype(
                    str).values.tolist()
    ...
    add_input_table("Relevant inputs",
                    [("Load profiles included", ", ".join([ui_name(c) for c in load_cols]) if load_cols else "n/a"),
                     ("Number of load profiles", str(len(load_cols))), ("Project area", f"{project_area_r:,.1f} m²")])
    ...
    try:
        ...
        bench_kpis = [("Net EUI", f"{eui_net:,.1f} kWh/m²·a"), ("Net CO₂ intensity", f"{co2_net:,.1f} kgCO₂/m²·a"),
                      ("Net energy cost", f"{currency_r} {cost_net:,.2f}/m²·a")]
        add_kpi_table("Benchmark KPIs", bench_kpis)
        bench_df = pd.DataFrame({"KPI": ["EUI", "CO₂", "Cost"], "Value": [eui_net, co2_net, cost_net]})
        _report_add_chart(story, styles, "Benchmark KPI overview",
                          _report_bar_chart(bench_df, "KPI", "Value", "Project KPI Values", "Intensity / cost",
                                            CRREM_COLOR_BASELINE),
                          "Benchmark categories depend on the selected building use and the benchmark template available in the app.")
        if benchmark_df is not None and not benchmark_df.empty:
            story.append(Paragraph("Benchmark data source loaded successfully for the selected building use.",
                                   styles["BodyText"]))
        else:
            story.append(Paragraph(
                "No benchmark threshold sheet was found for the selected building use. The report therefore shows project KPI values only.",
                styles["BodyText"]))
    except Exception:
        add_kpi_table("Benchmark KPIs", [("Net EUI", f"{net_eui_eff:,.1f} kWh/m²·a"),
                                         ("Net CO₂ intensity", f"{net_co2_int:,.1f} kgCO₂/m²·a"),
                                         ("Net energy cost", f"{currency_r} {net_cost_int:,.2f}/m²·a")])
    ...
    if crrem is None:
        story.append(
            Paragraph("CRREM dataset was not found. CRREM diagrams could not be generated.", styles["BodyText"]))
    else:
        ...
        max_year = int(
            min(int(project_year_r + analysis_period - 1), int(ef_grid.index.max()))) if ef_grid is not None and len(
            ef_grid) else int(project_year_r + analysis_period - 1)
        years_crrem = list(range(int(project_year_r), max_year + 1))
        carbon_limit, eui_limit = _report_crrem_limits_for_context(crrem, target_id, crrem_use, mixed_records,
                                                                   years_crrem)
        if not carbon_limit.empty:
            years_crrem = list(carbon_limit.index.astype(int))
        annual_em_t = compute_crrem_like_scenario_emissions_series_cached(df_energy, payload, crrem, project_year_r,
                                                                          years_crrem).reindex(years_crrem).fillna(0.0)
        ...
        _report_add_chart(story, styles, "Carbon intensity vs CRREM pathway",
                          _report_line_chart({"Project": carbon_asset, "CRREM limit": carbon_limit},
                                             "Carbon Intensity vs CRREM", "kgCO₂e/m²·a",
                                             {"Project": scenario_color, "CRREM limit": CRREM_COLOR_LIMIT},
                                             dashed={"CRREM limit"}),
                          "Project annual emissions use CRREM decarbonization logic. The CRREM limit is the selected pathway for the project country and use type.")
        _report_add_chart(story, styles, "EUI vs CRREM pathway",
                          _report_line_chart({"Project": eui_asset, "CRREM limit": eui_limit}, "EUI vs CRREM",
                                             "kWh/m²·a", {"Project": scenario_color, "CRREM limit": CRREM_COLOR_LIMIT},
                                             dashed={"CRREM limit"}),
                          "Project EUI is the annual factored consumption intensity. The CRREM limit is taken from the pathway dataset.")
        ...
        _report_add_chart(story, styles, "Total annual emissions",
                          _report_line_chart({"Project": total_em_t, "CRREM limit": total_em_limit_t},
                                             "Total Annual Emissions", "tCO₂e/a",
                                             {"Project": scenario_color, "CRREM limit": CRREM_COLOR_LIMIT},
                                             dashed={"CRREM limit"}),
                          "Annual emissions are converted from intensity to total emissions using project area.")
        _report_add_chart(story, styles, "Cumulative emissions", _report_line_chart(
            {"Project cumulative": total_em_t.cumsum(), "CRREM cumulative limit": total_em_limit_t.cumsum()},
            "Cumulative Emissions", "tCO₂e",
            {"Project cumulative": scenario_color, "CRREM cumulative limit": CRREM_COLOR_LIMIT},
            dashed={"CRREM cumulative limit"}),
                          "Cumulative emissions are the running sum of annual decarbonized emissions.")
        try:
            payload_without_measures = deepcopy(payload)
            payload_without_measures["crrem_measures"] = []
            annual_em_without_t = compute_crrem_like_scenario_emissions_series_cached(df_energy,
                                                                                      payload_without_measures, crrem,
                                                                                      project_year_r,
                                                                                      years_crrem).reindex(
                years_crrem).fillna(0.0)
            annual_em_with_t = compute_crrem_like_scenario_emissions_series_cached(df_energy, payload, crrem,
                                                                                   project_year_r, years_crrem).reindex(
                years_crrem).fillna(0.0)
            annual_em_savings_t = annual_em_without_t - annual_em_with_t
            cumulative_em_savings_t = annual_em_savings_t.cumsum()
            eui_without = compute_crrem_like_scenario_eui_series(df_energy, payload_without_measures, project_year_r,
                                                                 years_crrem, project_area_r).reindex(
                years_crrem).fillna(0.0)
            eui_with = compute_crrem_like_scenario_eui_series(df_energy, payload, project_year_r, years_crrem,
                                                              project_area_r).reindex(years_crrem).fillna(0.0)
            ...
            _report_add_chart(
                ...
                _report_line_chart({"Annual emission savings": annual_em_savings_t}, "Annual Emission Savings",
                                   "tCO₂e/a", {"Annual emission savings": CRREM_COLOR_MEASURES}),
            ...
            _report_add_chart(
                ...
                _report_line_chart({"Cumulative emission savings": cumulative_em_savings_t},
                                   "Cumulative Emission Savings", "tCO₂e",
                                   {"Cumulative emission savings": CRREM_COLOR_MEASURES}),
            ...
            _report_add_chart(
                ...
                _report_line_chart({"Annual end energy savings": annual_energy_savings_mwh},
                                   "Annual End Energy Savings", "MWh/a",
                                   {"Annual end energy savings": CRREM_COLOR_MEASURES}),
            ...
            _report_add_chart(
                ...
                _report_line_chart({"Cumulative end energy savings": cumulative_energy_savings_mwh},
                                   "Cumulative End Energy Savings", "MWh",
                                   {"Cumulative end energy savings": CRREM_COLOR_MEASURES}),
                "Cumulative sum of annual end-energy savings. Positive values indicate cumulative reduced end energy demand."
            )
        except Exception:
            story.append(
                Paragraph("CRREM savings diagrams could not be generated for the active scenario.", styles["BodyText"]))
    add_input_table("Relevant inputs", [("Country", project_country_r), ("Project year", project_year_r), (
    "Emission factors", ", ".join([f"{k}: {v:.4f}" for k, v in factor_map.items()])),
                                        ("CRREM target", str(st.session_state.get("crrem_target_select", "1.5°C")))])
    ...
    if cf.empty:
        story.append(Paragraph(
            "No LCC cash-flow data available. Add LCC assumptions and investment measures in the LCC-Analysis tab.",
            styles["BodyText"]))
    else:
        ...
        if ref_name and ref_name in scenarios_for_lcc_report and ref_name != active_name:
            try:
                ...
                ref_loads = get_loads_balance_df(file_bytes, filename, scenario_name=ref_name,
                                                 apply_master_filter=False)
        ...
        add_kpi_table("LCC KPIs", [
            ...
            ("Discounted LCC intensity",
             f"{currency_r} {total_disc / project_area_r:,.2f}/m²" if project_area_r else "n/a"),
            ("Discounted payback period", _format_payback(pb)),
        ])
        type_df = by_type.reset_index().rename(columns={"Nominal Cost": "Cost"})
        _report_add_chart(story, styles, "Annual LCC balance",
                          _report_stacked_monthly_chart(cf.rename(columns={"Year": "Month"}), "Nominal Cost",
                                                        "Cost Type", "Annual LCC Balance", f"{currency_r}/a",
                                                        LCC_COST_TYPE_COLORS),
                          "Annual LCC balance stacks energy, investment, maintenance and replacement costs by year.")
        lcc_report_series = {
        ...
        if isinstance(ref_cf, pd.DataFrame) and not ref_cf.empty and ref_name:
            try:
                ref_by_year = ref_cf.groupby("Year", as_index=True).agg(
                    {"Nominal Cost": "sum", "Discounted Cost": "sum"})
                ...
                ref_color = colors_scenarios.get(str(ref_name), "#9ca3af") if isinstance(colors_scenarios,
                                                                                         dict) else "#9ca3af"
                ...
                pass
        _report_add_chart(story, styles, "Cumulative LCC",
                          _report_line_chart(lcc_report_series, "Cumulative LCC", currency_r, lcc_report_colors,
        ...
        _report_cost_type_values = (by_type["Nominal Cost"] / project_area_r) if project_area_r else by_type[
            "Nominal Cost"]
        _report_cost_type_center = f"{currency_r} {total_nom / project_area_r:,.2f}\n/m²" if project_area_r else f"{currency_r} {total_nom:,.0f}"
        _report_add_chart(story, styles, "LCC by cost type",
                          _report_pie_chart(_report_cost_type_values, "Nominal LCC per m² by Cost Type",
                                            _report_cost_type_center, LCC_COST_TYPE_COLORS),
                          "Cost-type shares use nominal LCC intensity over the analysis period.")
        _report_add_chart(story, styles, "LCC by assigned end use",
                          _report_pie_chart(by_end / project_area_r if project_area_r else by_end,
                                            "Nominal LCC per m² by End Use",
                                            f"{currency_r} {total_nom / project_area_r:,.0f}\n/m²" if project_area_r else f"{currency_r} {total_nom:,.0f}",
                                            colors_eu),
                          "Costs assigned to multiple end uses are allocated equally across the assigned end uses.")
    add_input_table("Relevant inputs", [
        ...
        ("Operational end-use filter",
         ", ".join([ui_name(u) for u in lcc_global.get("selected_operational_end_uses", [])])),
        ("Discounted payback reference", ref_name if ref_name else "None"),
        ("Energy inflation", ", ".join(
            [f"{src}: {float((lcc_global.get('energy_inflation_pct', {}) or {}).get(src, 0.0)):,.2f}%" for src in
             ENERGY_SOURCE_ORDER])),
    ])
    ...
    try:
        ...
        if scenario_order_report:
            ...
            radar_ref_report = str(
                st.session_state.get("scenario_radar_reference_scenario", active_name) or active_name)
            ...
            if not radar_improvement_report.empty:
                _report_add_chart(
                    ...
                    _report_radar_chart(radar_improvement_report, "Relative Improvement Radar", colors_scenarios,
                                        scenario_order_report, mode="improvement"),
                ...
                _report_add_chart(
                    ...
                    _report_radar_chart(radar_absolute_report, "Axis-Scaled Absolute KPI Radar", colors_scenarios,
                                        scenario_order_report, mode="absolute"),
                ...
                if isinstance(radar_axis_report, pd.DataFrame) and not radar_axis_report.empty:
                    ...
                    story.append(_report_table_flowable(radar_axis_rows, col_widths=[6.5 * cm, 4.5 * cm, 4.5 * cm],
                                                        font_size=6.5))
                ...
                _report_add_chart(
                    ...
                    _report_scenario_kpi_scatter_chart(radar_raw_report, "Benchmark-Style Scenario KPI Bar",
                                                       colors_scenarios, scenario_order_report),
                ...
                delta_ref_report = str(
                    st.session_state.get("scenario_delta_reference_scenario", radar_ref_report) or radar_ref_report)
                if delta_ref_report not in scenario_order_report:
                    delta_ref_report = radar_ref_report if radar_ref_report in scenario_order_report else \
                    scenario_order_report[0]
                ...
                if isinstance(delta_long_report, pd.DataFrame) and not delta_long_report.empty:
                    _report_add_chart(
                        ...
                        _report_scenario_delta_improvement_chart(delta_long_report,
                                                                 f"KPI Improvement vs {delta_ref_report}",
                                                                 colors_scenarios, scenario_order_report),
                    ...
                    if isinstance(delta_summary_report, pd.DataFrame) and not delta_summary_report.empty:
                        _report_add_chart(
                            ...
                            _report_scenario_delta_mac_chart(delta_summary_report, "Marginal Abatement Cost",
                                                             colors_scenarios, scenario_order_report),
                        ...
                        for _, _dr in delta_summary_report.iterrows():
                            delta_rows_report.append([
                                ...
                                _report_format_number(_dr.get("Marginal abatement cost (currency/tCO₂e)", np.nan),
                                                      decimals=0),
                            ])
                        story.append(_report_table_flowable(delta_rows_report,
                                                            col_widths=[4.0 * cm, 4.3 * cm, 3.5 * cm, 3.5 * cm],
                                                            font_size=6.3))
                        story.append(Spacer(1, 0.2 * cm))
    except Exception:
        pass
    story.append(Paragraph(
        "The report is generated for the active scenario only for the detailed scenario charts below. The radar summary compares all available scenarios.",
        styles["BodyText"]))
    try:
    ...
    if isinstance(cf_scenario_report, pd.DataFrame) and not cf_scenario_report.empty:
        by_year = cf_scenario_report.groupby("Year", as_index=True).agg(
            {"Nominal Cost": "sum", "Discounted Cost": "sum"})
        energy_by_year = cf_scenario_report.loc[cf_scenario_report["Cost Type"] == "Energy"].groupby("Year")[
            "Nominal Cost"].sum().reindex(by_year.index).fillna(0.0)
        _report_add_chart(story, styles, "Annual energy cost",
                          _report_line_chart({active_name: energy_by_year}, "Annual Energy Cost", f"{currency_r}/a",
                                             {active_name: scenario_color}),
                          "Annual energy cost is calculated from active-scenario factored kWh, tariffs and energy inflation.")
        _report_add_chart(story, styles, "Cumulative LCC", _report_line_chart(
            {"Nominal": by_year["Nominal Cost"].cumsum(), "Discounted": by_year["Discounted Cost"].cumsum()},
            "Cumulative LCC - Active Scenario", currency_r, {"Nominal": scenario_color, "Discounted": scenario_color},
            dashed={"Discounted"}), "Solid line is nominal cumulative cost; dashed line is discounted cumulative cost.")
    if crrem is not None:
        try:
            ...
            annual_em_sc = compute_crrem_like_scenario_emissions_series_cached(df_energy, payload, crrem,
                                                                               project_year_r, years_sc).reindex(
                years_sc).fillna(0.0)
            target_id = "1.5C" if str(st.session_state.get("crrem_target_select", "1.5°C")).startswith("1.5") else "2C"
            carbon_limit_sc, _ = _report_crrem_limits_for_context(crrem, target_id,
                                                                  str(payload.get("crrem_use_type", "Office")),
                                                                  payload.get("crrem_mixed_use", []), years_sc)
            crrem_total = carbon_limit_sc * project_area_r / 1000.0 if not carbon_limit_sc.empty else pd.Series(
                dtype=float)
            _report_add_chart(story, styles, "Annual emissions",
                              _report_line_chart({active_name: annual_em_sc, "CRREM-Baseline": crrem_total},
                                                 "Annual Emissions - Active Scenario", "tCO₂e/a",
                                                 {active_name: scenario_color, "CRREM-Baseline": CRREM_COLOR_LIMIT},
                                                 dashed={"CRREM-Baseline"}),
                              "Annual emissions use the same decarbonization pathway logic as the CRREM tab.")
            _report_add_chart(story, styles, "Cumulative emissions", _report_line_chart(
                {active_name: annual_em_sc.cumsum(), "CRREM-Baseline": crrem_total.cumsum()},
                "Cumulative Emissions - Active Scenario", "tCO₂e",
                {active_name: scenario_color, "CRREM-Baseline": CRREM_COLOR_LIMIT}, dashed={"CRREM-Baseline"}),
                              "Cumulative emissions are the running sum of annual decarbonized emissions.")
        except Exception:
            pass
    add_input_table("Relevant inputs", [
        ...
        ("Scenario-specific raw Energy_Balance override",
         "Yes" if get_scenario_energy_balance_override(active_name) is not None else "No"),
    ])
    ...
    try:
        ...
        story.append(Paragraph(
            "The report includes global model setup inputs and scenario-specific model inputs for the active scenario only. Assumption-tagged inputs should be reviewed and replaced with documented project references where possible.",
            styles["BodyText"]))
        story.append(Spacer(1, 0.25 * cm))
        for cat in MODEL_INPUT_CATEGORIES:
            ...
            for (scope_i, item_type_i, item_name_i), sub in sub_cat.groupby(["Scope", "Item Type", "Item Name"],
                                                                            dropna=False):
                ...
                story.append(_report_table_flowable(rows_mi,
                                                    col_widths=[3.5 * cm, 2.5 * cm, 1.5 * cm, 2.6 * cm, 2.2 * cm,
                                                                3.2 * cm], font_size=5.5))
                story.append(Spacer(1, 0.15 * cm))
    except Exception:
        story.append(Paragraph("Model Inputs QA data could not be loaded for this report.", styles["BodyText"]))
    add_input_table("Relevant inputs", [("Register source", "Model_Inputs_QA workbook sheet / Model Inputs QA tab"),
                                        ("Global inputs", "General Model Setup"),
                                        ("Scenario-specific inputs", "Room types, envelope components and systems"),
                                        ("Assumption tag", "Source Type = Assumption")])
...
def load_scenario_into_widgets(payload: dict, end_uses: list) -> None:
    ...
    try:
        ...
        st.session_state["crrem_target_select"] = _crrem_target_loaded if _crrem_target_loaded in {"1.5°C",
                                                                                                   "2°C"} else "1.5°C"
    except Exception:
...
def default_model_inputs_global_rows() -> list:
    ...
    return [
        _mi_row(MODEL_INPUT_GLOBAL_SCENARIO, "Global", "General Model Setup", "General", "Simulation Model",
                "Simulation tool", unit="-", required=True, source_type="Simulation Model",
                reference="e.g. IESVE / EnergyPlus / DesignBuilder"),
        _mi_row(MODEL_INPUT_GLOBAL_SCENARIO, "Global", "General Model Setup", "General", "Simulation Model",
                "Simulation tool version", unit="-", required=True, source_type="Simulation Model",
                reference="Version used for the final simulation results"),
        _mi_row(MODEL_INPUT_GLOBAL_SCENARIO, "Global", "General Model Setup", "General", "Simulation Model",
                "Weather file", unit="-", required=True, source_type="Simulation Model",
                reference="Weather station/file used in annual simulation"),
        _mi_row(MODEL_INPUT_GLOBAL_SCENARIO, "Global", "General Model Setup", "General", "Simulation Model",
                "Climate zone", unit="-", required=False, source_type="Standard / Reference",
                source_ref="ASHRAE / LEED / local code documentation",
                reference="Required when comparing against a formal baseline"),
        _mi_row(MODEL_INPUT_GLOBAL_SCENARIO, "Global", "General Model Setup", "General", "Simulation Model",
                "Simulation timestep", unit="min", required=False, source_type="Simulation Model",
                reference="Typically 60 min or sub-hourly", min_check=1, max_check=60),
        _mi_row(MODEL_INPUT_GLOBAL_SCENARIO, "Global", "General Model Setup", "General", "Simulation Model",
                "Annual simulation period", unit="months", required=True, source_type="Simulation Model",
                reference="Should normally cover 12 months", min_check=12, max_check=12),
        _mi_row(MODEL_INPUT_GLOBAL_SCENARIO, "Global", "General Model Setup", "General", "Simulation Model",
                "Modelled floor area", unit="m²", required=True, source_type="Design Document",
                source_ref="Area schedule / simulation model geometry",
                reference="Should match project area or document difference", min_check=1),
        _mi_row(MODEL_INPUT_GLOBAL_SCENARIO, "Global", "General Model Setup", "General", "Simulation Model",
                "Area basis", unit="-", required=False, source_type="Design Document",
                reference="GFA / NFA / conditioned area / treated floor area"),
        _mi_row(MODEL_INPUT_GLOBAL_SCENARIO, "Global", "General Model Setup", "General", "Simulation Model",
                "Geometry / zoning source", unit="-", required=True, source_type="Design Document",
                source_ref="Architectural drawings / BIM / simulation model",
                reference="Trace model version and drawing issue"),
        _mi_row(MODEL_INPUT_GLOBAL_SCENARIO, "Global", "General Model Setup", "General", "Simulation Model",
                "Simulation purpose / rating basis", unit="-", required=False, source_type="Standard / Reference",
                reference="Design support / LEED / ASHRAE 90.1 PRM / local code / internal benchmark"),
        _mi_row(MODEL_INPUT_GLOBAL_SCENARIO, "Global", "General Model Setup", "General", "Simulation Model",
                "Excluded areas / systems", unit="-", required=False, source_type="Design Document",
                source_ref="Energy model report / compliance narrative",
                reference="Document any excluded systems or loads"),
        _mi_row(MODEL_INPUT_GLOBAL_SCENARIO, "Global", "General Model Setup", "General", "Simulation Model",
                "Modeller / reviewer", unit="-", required=False, source_type="Other",
                reference="Responsible person and QA reviewer"),
    ]
...
def room_type_template(item_name: str, scenario: str, scope: str = "Global") -> list:
    ...
    return [
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Area", unit="m²", required=True,
                source_type="Design Document", source_ref="Area schedule / room book", min_check=0),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Occupancy density", unit="m²/person",
                required=False, source_type="Design Document",
                source_ref="Room data sheet / LEED calculator / design brief", min_check=1, max_check=100),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Lighting power density", unit="W/m²",
                required=True, source_type="Design Document",
                source_ref="Lighting concept / ASHRAE baseline / design brief", min_check=0, max_check=50),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Equipment power density", unit="W/m²",
                required=True, source_type="Design Document", source_ref="Equipment schedule / design brief",
                min_check=0, max_check=150),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "People sensible gain", unit="W/person",
                required=False, source_type="Assumption", reference="Document activity/metabolic assumption",
                min_check=0, max_check=200),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "People latent gain", unit="W/person",
                required=False, source_type="Assumption", reference="Document activity/metabolic assumption",
                min_check=0, max_check=200),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Heating delivery", value="", unit="-",
                required=False, source_type="Design Document", source_ref="HVAC concept / room data sheet",
                reference="Radiant Ceiling / Fan Coil / Floor Heating / Radiator / Air System / custom"),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Cooling delivery", value="", unit="-",
                required=False, source_type="Design Document", source_ref="HVAC concept / room data sheet",
                reference="Radiant Ceiling / Fan Coil / Floor Cooling / Fan Coil / Air System / custom"),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Heating setpoint", unit="°C", required=True,
                source_type="Design Document", source_ref="Owner requirements / room data sheet", min_check=10,
                max_check=26),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Cooling setpoint", unit="°C", required=True,
                source_type="Design Document", source_ref="Owner requirements / room data sheet", min_check=18,
                max_check=35),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Night setback / setup", unit="°C or rule",
                required=False, source_type="Assumption", reference="Document unoccupied temperature control logic"),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Occupancy schedule", unit="-", required=True,
                source_type="Design Document", source_ref="Operation concept / design brief",
                reference="Weekday/weekend and holiday operation"),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Lighting schedule", unit="-", required=True,
                source_type="Assumption", source_ref="Operation concept / lighting controls",
                reference="Equivalent full-load hours or schedule name"),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Daylight-Controlled dimming", value="No",
                unit="yes/no", required=False, source_type="Design Document",
        ...
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Equipment schedule", unit="-", required=True,
                source_type="Assumption", source_ref="Operation concept / equipment load assumptions",
                reference="Equivalent full-load hours or schedule name"),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Outdoor air per person", unit="L/s.person",
                required=False, source_type="Standard / Reference", source_ref="ASHRAE 62.1 / EN 16798 / project brief",
                min_check=0, max_check=50),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Outdoor air per area", unit="L/s.m²",
                required=False, source_type="Standard / Reference", source_ref="ASHRAE 62.1 / EN 16798 / project brief",
                min_check=0, max_check=10),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Demand controlled ventilation permitted",
                unit="yes/no", required=False, source_type="Design Document", source_ref="Controls narrative"),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Demand controlled ventilation rule", unit="-",
                required=False, source_type="Design Document", source_ref="Controls narrative",
                reference="CO₂ setpoint, minimum outdoor air, occupancy sensor logic"),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Natural ventilation permitted", unit="yes/no",
                required=False, source_type="Design Document", source_ref="Ventilation / façade / controls concept"),
        _mi_row(sc, scope_value, "Room Types", "Room Type", item_name, "Natural ventilation rule", unit="-",
                required=False, source_type="Design Document", source_ref="Ventilation / controls concept",
                reference="Opening schedule, temperature limits, CO₂ limits, wind/rain lockout"),
    ]
...
def envelope_component_template(component_type: str, item_name: str, scenario: str, scope: str = "Scenario") -> list:
    ...
    if ct == "External Walls":
        params = [
            ("Area", "m²", False, "Design Document",
             "Envelope schedule / model geometry; or enter a location/use descriptor such as Underground",
             "Optional: numeric area or generic location where this construction is used", 0, None),
            ("U-value", "W/m²K", True, "Design Document", "U-value calculation / envelope specification",
             "Compare with code/baseline", 0.05, 2.5),
            ("Thermal bridge allowance", "% or W/K", False, "Calculation", "Envelope calculation", "Document method", 0,
             50),
            ("Thermal Mass", "-", False, "Design Document", "Construction build-up / material layer schedule",
             "Very low / low / medium / high / very high, or enter thermal capacity as custom value", None, None),
        ...
        params = [
            ("Area", "m²", False, "Design Document",
             "Envelope schedule / model geometry; or enter a location/use descriptor such as Typical roof / podium roof",
             "Optional: numeric area or generic location where this construction is used", 0, None),
            ("U-value", "W/m²K", True, "Design Document", "U-value calculation / envelope specification",
             "Compare with code/baseline", 0.04, 1.5),
            ("Thermal bridge allowance", "% or W/K", False, "Calculation", "Envelope calculation", "Document method", 0,
             50),
            ("Thermal Mass", "-", False, "Design Document", "Construction build-up / material layer schedule",
             "Very low / low / medium / high / very high, or enter thermal capacity as custom value", None, None),
            ("Solar absorptance", "-", False, "Assumption", "Roof finish specification", "", 0, 1),
            ("Green roof / cool roof assumption", "yes/no/rule", False, "Design Document", "Roof concept", "", None,
             None),
        ]
    elif ct == "Floors / Slabs":
        params = [
            ("Area", "m²", False, "Design Document",
             "Envelope schedule / model geometry; or enter a location/use descriptor such as Underground / against ground",
             "Optional: numeric area or generic location where this construction is used", 0, None),
            ("U-value", "W/m²K", True, "Design Document", "U-value calculation / envelope specification",
             "Compare with code/baseline", 0.04, 2.0),
            ("Thermal bridge allowance", "% or W/K", False, "Calculation", "Envelope calculation", "Document method", 0,
             50),
            ("Thermal Mass", "-", False, "Design Document", "Construction build-up / material layer schedule",
             "Very low / low / medium / high / very high, or enter thermal capacity as custom value", None, None),
        ...
        params = [
            ("Area", "m²", False, "Design Document",
             "Façade schedule / model geometry; or enter a location/use descriptor such as North façade / Atrium",
             "Optional: numeric area or generic location where this glazing type is used", 0, None),
            ("U-value", "W/m²K", True, "Design Document", "Glazing specification", "Compare with code/baseline", 0.5,
             6.0),
            ...
            ("Associated shading device", "-", False, "Design Document", "Shading concept / façade schedule",
             "Select an existing Shading Device object where applicable", None, None),
            ("Shading device / control", "-", False, "Design Document", "Shading concept / controls",
             "Internal/external, fixed/dynamic, schedule/rule", None, None),
        ]
    elif ct == "Shading Devices":
        params = [
            ("Shading type", "-", True, "Design Document", "Façade / shading concept",
             "Fixed, movable, electrochromic, internal blind, etc.", None, None),
            ("Geometry / projection", "m or rule", False, "Design Document", "Façade drawings",
             "Overhang/fin depth, spacing, angle", None, None),
            ("Control rule", "-", False, "Design Document", "Controls narrative",
             "Solar radiation, glare, schedule, temperature, manual", None, None),
            ("Reduction factor", "-", False, "Assumption", "Simulation model", "Document if simplified", 0, 1),
        ]
    elif ct == "Infiltration / Airtightness":
        params = [
            ("Infiltration rate", "1/h or L/s.m²", True, "Assumption", "Airtightness concept / model input",
             "Flag assumption unless measured/specified", 0, 5),
            ("Infiltration schedule / rule", "-", False, "Assumption", "Simulation model", "Wind/stack/schedule logic",
             None, None),
            ("Airtightness test value", "n50 or q50", False, "Measurement", "Blower-door test / specification", "", 0,
             20),
        ]
    else:
        params = [
            ("Area / quantity", "m² or unit", False, "Design Document", "Project documentation", "", 0, None),
            ("Main performance value", "project unit", False, "Assumption", "Project documentation",
             "Document unit and method", None, None),
            ("Modelling rule", "-", False, "Simulation Model", "Energy model input", "", None, None),
        ]
    return [_mi_row(sc, scope_value, "Thermal Envelope", ct, item_name, p, unit=u, required=req, source_type=src,
                    source_ref=ref, reference=tgt, min_check=mn, max_check=mx) for p, u, req, src, ref, tgt, mn, mx in
            params]
...
def system_template(system_type: str, item_name: str, scenario: str, scope: str = "Scenario") -> list:
    ...
    if stype == "AHU / Ventilation":
        params = [
            ("Served Room Types", "room types", True, "Design Document", "MEP concept / zone list",
             "Select one or more existing Room Type objects served by this AHU", None, None),
            ...
            (
            "Heat recovery type", "-", False, "Design Document", "AHU specification", "Plate, rotary, run-around, none",
            None, None),
            ("Heat recovery efficiency", "%", False, "Design Document", "AHU specification",
             "Sensible/total effectiveness used in model", 0, 95),
            ("Specific fan power", "W/(L/s)", False, "Design Document", "Fan/AHU schedule",
             "Check against selected standard/project target", 0, 8),
            ("Supply air temperature", "°C", False, "Design Document", "Controls sequence / AHU schedule", "", 10, 35),
            ("AHU operation schedule", "-", True, "Design Document", "Controls narrative / operation concept", "", None,
             None),
            ("Demand controlled ventilation", "yes/no", False, "Design Document", "Controls narrative", "", None, None),
            ("Demand controlled ventilation rule", "-", False, "Design Document", "Controls narrative",
             "CO₂ setpoint, occupancy sensor, min OA", None, None),
            ("Free cooling / economizer rule", "-", False, "Design Document", "Controls narrative",
             "Outdoor air temperature/enthalpy limits", None, None),
            ("Humidification / dehumidification", "yes/no/rule", False, "Design Document", "MEP concept / controls", "",
             None, None),
        ]
    elif stype == "Heating":
        params = [
            ("System type", "-", True, "Design Document", "MEP concept", "Boiler, heat pump, district heating, etc.",
             None, None),
            ("Energy source", "-", True, "Design Document", "MEP concept / energy source mapping",
             "Must be consistent with Energy_Balance mapping", None, None),
            ("Overall efficiency / seasonal COP", "-", True, "Design Document", "Equipment datasheet / model input",
             "Boiler efficiency or heat-pump SCOP", 0, 10),
            ...
            ("Control rule", "-", False, "Design Document", "Controls narrative",
             "Weather compensation, setback, room control", None, None),
            ("Distribution / storage losses", "% or kWh/a", False, "Assumption", "MEP calculation / model input",
             "Document method", 0, 50),
        ]
    elif stype == "Cooling":
        params = [
            ("System type", "-", True, "Design Document", "MEP concept",
             "Chiller, heat pump, district cooling, VRF, etc.", None, None),
            ("Energy source", "-", True, "Design Document", "MEP concept / energy source mapping",
             "Must be consistent with Energy_Balance mapping", None, None),
            ("Overall efficiency / seasonal COP", "-", True, "Design Document", "Equipment datasheet / model input",
             "Chiller COP, SEER, EER, DC assumption", 0, 15),
        ...
        params = [
            ("System type", "-", True, "Design Document", "DHW concept",
             "Boiler, heat pump, district heating, electric, solar thermal", None, None),
            ...
            ("Hot Water Demand", "L/person·day", True, "Calculation",
             "DHW calculation / design brief / plumbing fixture schedule",
             "Document basis: persons, area, fixtures, schedule or annual volume", 0, None),
            ("DHW demand calculation method", "-", False, "Calculation", "DHW calculation / design brief",
             "Persons, fixtures, litres/day, schedule", None, None),
    ...
    return [_mi_row(sc, scope_value, stype if stype in MODEL_INPUT_CATEGORIES else "Other / Custom Inputs", stype,
                    item_name, p, unit=u, required=req, source_type=src, source_ref=ref, reference=tgt, min_check=mn,
                    max_check=mx) for p, u, req, src, ref, tgt, mn, mx in params]
...
def sanitize_model_inputs_qa_df(df: Optional[pd.DataFrame]) -> pd.DataFrame:
    ...
    cat_series = out["Category"].astype(str) if "Category" in out.columns else pd.Series([""] * len(out),
                                                                                         index=out.index)
    ...
    if "Item Name" not in out.columns:
        out["Item Name"] = np.where(cat_series.eq("General Model Setup"), "Simulation Model",
                                    cat_series.replace("", "Custom"))
    ...
    for col in ["Scenario", "Scope", "Category", "Item Type", "Item Name", "Parameter", "Value", "Unit", "Source Type",
                "Source Document / Reference", "Reference / Target", "Range Justification", "Notes"]:
    ...
    out.loc[out["Parameter"].astype(str).eq("Served Room Types") & out["Unit"].astype(str).str.strip().eq(
        ""), "Unit"] = "room types"
    ...
    try:
        _env_area_mask = out["Category"].astype(str).eq("Thermal Envelope") & out["Parameter"].astype(str).isin(
            ["Area", "Area / quantity"])
        out.loc[_env_area_mask, "Required"] = False
        out.loc[_env_area_mask & out["Reference / Target"].astype(str).str.strip().eq(
            ""), "Reference / Target"] = "Optional: numeric area or generic location where this construction/component is used"
    except Exception:
        pass
    ...
    existing_global_params = set(
        out.loc[out["Scenario"].astype(str).eq(MODEL_INPUT_GLOBAL_SCENARIO), "Parameter"].astype(str))
        ...
        for _, item in object_keys.iterrows():
            ...
            if not has_id:
                id_rows.append(_mi_row(
                    sc,
                    scope if scope in ["Global", "Scenario"] else (
                        "Global" if sc == MODEL_INPUT_GLOBAL_SCENARIO else "Scenario"),
                    cat,
                    it,
                    nm,
                    "ID",
...
def model_inputs_scenario_differences(df: Optional[pd.DataFrame], scenario_names: list) -> Tuple[
    ...
    for _, row in out.iterrows():
        ...
        if num is not None:
            ...
            if status != "Missing" and _is_model_input_out_of_usual_range(val_raw, usual_min, usual_max):
                ...
                msg_parts.append(
                    f"outside usual range ({_format_usual_range(usual_min, usual_max, row.get('Unit', ''))})")
...
def _model_input_ensure_unique_name(existing_rows: list, item_key: tuple, requested_name: str = "",
                                    allow_current: bool = False) -> str:
...
def add_model_envelope_component(component_type: str, item_name: str, scenario_name: str,
                                 scope: str = "Scenario") -> None:
    nm = str(item_name or component_type or "Component").strip() or "Component"
    add_model_input_rows(
        envelope_component_template(str(component_type or "Other Envelope Component"), nm, str(scenario_name or "Base"),
                                    scope=scope))
...
def add_model_system(system_type: str, item_name: str, scenario_name: str, scope: str = "Scenario") -> None:
    ...
    add_model_input_rows(
        system_template(str(system_type or "Other System"), nm, str(scenario_name or "Base"), scope=scope))


def add_model_custom_parameter(scenario_name: str, scope: str, category: str, item_type: str, item_name: str,
                               parameter_name: str) -> None:
    sc = MODEL_INPUT_GLOBAL_SCENARIO if str(scope).lower() == "global" else str(scenario_name or "Base")
    add_model_input_rows([_mi_row(sc, "Global" if sc == MODEL_INPUT_GLOBAL_SCENARIO else "Scenario", category,
                                  item_type, item_name, str(parameter_name or "Custom parameter"), unit="-",
                                  required=False, source_type="Assumption")])
...
def _coerce_crrem_ef_payload(settings: Optional[dict]) -> dict:
    ...
    if isinstance(settings, dict):
        out["use_standard_electricity"] = bool(
            settings.get("use_standard_electricity", out["use_standard_electricity"]))
        ...
        if isinstance(raw_rates, dict):
            for src in ENERGY_SOURCE_ORDER:
                try:
                    out["custom_decarb_rates_pct"][src] = float(
                        str(raw_rates.get(src, out["custom_decarb_rates_pct"].get(src, 0.0))).replace(",", "."))
                except Exception:
                    out["custom_decarb_rates_pct"][src] = 0.0

    # Keep aggregate flag in sync with per-source flags for compatibility/export.
    try:
        out["use_custom_decarb_rates"] = any(
            bool(out["use_custom_decarb_by_source"].get(src, False)) for src in ENERGY_SOURCE_ORDER)
    except Exception:
    ...
    for sc_name, payload in scenarios.items():
        ...
        for src in ENERGY_SOURCE_ORDER:
            rows.append({
                ...
                "Use_Custom_Decarbonization_Rate_For_Source": bool(
                    settings.get("use_custom_decarb_by_source", {}).get(str(src), False)),
                ...
                "Custom_Decarbonization_Rate_pct_a": float(
                    settings.get("custom_decarb_rates_pct", {}).get(str(src), 0.0) or 0.0),
            })
    ...
    for sc_name, group in df.groupby("Scenario"):
        ...
        try:
            if "Use_CRREM_Standard_Electricity_EF" in group.columns:
                settings["use_standard_electricity"] = _bool_from_cell(
                    group["Use_CRREM_Standard_Electricity_EF"].iloc[0], settings.get("use_standard_electricity", False))
        except Exception:
            pass
        ...
        try:
            if "Use_Custom_Decarbonization_Rates" in group.columns:
                legacy_global_custom = _bool_from_cell(group["Use_Custom_Decarbonization_Rates"].iloc[0],
                                                       legacy_global_custom)
        except Exception:
            pass
        ...
        for _, row in group.iterrows():
            ...
            if has_source_specific_flag_col:
                flags[src] = _bool_from_cell(row.get("Use_Custom_Decarbonization_Rate_For_Source", False),
                                             flags.get(src, False))
            else:
...
def compute_crrem_like_scenario_emissions_series(
    ...
    if not has_emission_relevant_measures:
        ...
        df_base_calc["Energy_Source"] = df_base_calc["End_Use"].map(
            lambda u: str(mapping.get(u, "Electricity"))).fillna("Electricity")
    ...
    def _ef_at_year(src: str, year: int) -> float:
        return _source_emission_factor_at_year(
            str(src), int(year), int(project_year), base_factors, ef_grid, crrem_ef_settings,
            ef_measures.get(str(src), [])
        )
...
if uploaded_file:
    ...
    saved_colors_enduse, saved_colors_sources, saved_colors_loads, saved_colors_scenarios = parse_color_settings_df(
        cfg_saved.get("colors"))
    ...
    if st.session_state.get(_RAW_TOKEN_KEY) != wb_token:
        ...
        st.session_state[_RAW_MAX_NUMBERED_SCENARIO_INDEX_KEY] = max(
            [1] + list(st.session_state[_RAW_ENERGY_NUMBERED_SHEETS_KEY].keys()) + list(
                st.session_state[_RAW_LOADS_NUMBERED_SHEETS_KEY].keys())
        )
    ...
    if _RAW_LOADS_DRAFT_KEY not in st.session_state and _RAW_LOADS_KEY in st.session_state:
        st.session_state[_RAW_LOADS_DRAFT_KEY] = st.session_state[_RAW_LOADS_KEY].copy(deep=True)
    if _RAW_ENERGY_SCENARIO_DIRTY_KEY not in st.session_state or not isinstance(
            st.session_state.get(_RAW_ENERGY_SCENARIO_DIRTY_KEY), dict):
        st.session_state[_RAW_ENERGY_SCENARIO_DIRTY_KEY] = {}
    if _RAW_LOADS_SCENARIO_DIRTY_KEY not in st.session_state or not isinstance(
            st.session_state.get(_RAW_LOADS_SCENARIO_DIRTY_KEY), dict):
        st.session_state[_RAW_LOADS_SCENARIO_DIRTY_KEY] = {}
    if _RAW_LOADS_SCENARIO_OVERRIDES_KEY not in st.session_state or not isinstance(
            st.session_state.get(_RAW_LOADS_SCENARIO_OVERRIDES_KEY), dict):
        st.session_state[_RAW_LOADS_SCENARIO_OVERRIDES_KEY] = {}
    if _RAW_LOADS_SCENARIO_DRAFTS_KEY not in st.session_state or not isinstance(
            st.session_state.get(_RAW_LOADS_SCENARIO_DRAFTS_KEY), dict):
        st.session_state[_RAW_LOADS_SCENARIO_DRAFTS_KEY] = {}
    if "model_inputs_qa_df" not in st.session_state or not isinstance(st.session_state.get("model_inputs_qa_df"),
                                                                      pd.DataFrame):
        try:
            st.session_state["model_inputs_qa_df"] = parse_model_inputs_qa_df(cfg_saved.get("model_inputs"))
        except Exception:
    ...
    if st.session_state.get("_loaded_workbook_token") != wb_token:
        ...
        if preloaded.get("scenario_comparison_period") is not None:
            try:
                st.session_state[SCENARIO_COMPARISON_PERIOD_KEY] = max(1, int(float(
                    preloaded.get("scenario_comparison_period", 50))))
            except Exception:
                pass

        try:
            st.session_state[SCENARIO_COMPARISON_APPLY_LCC_FILTER_KEY] = bool(
                preloaded.get("scenario_comparison_apply_lcc_filter", True))
        except Exception:
    ...
    if uploaded_file:
        ...
        if "scenarios" not in st.session_state:
            ...
            if scenarios_from_file:
                ...
                st.session_state["scenarios"] = scenarios_from_file
                st.session_state["active_scenario"] = "Base" if "Base" in scenarios_from_file else \
                list(scenarios_from_file.keys())[0]
            try:
        ...
        try:
            _bind_numbered_raw_sheets_to_scenarios(st.session_state.get("scenarios", {}),
                                                   wb_token if uploaded_file else None)
        except Exception:
            pass
        ...
        with st.sidebar.expander("Scenario Manager", expanded=True):
            ...
            active_idx = scenario_names.index(selector_value) if selector_value in scenario_names else (
                scenario_names.index(current_active) if current_active in scenario_names else 0)
            ...
            def _end_uses_for_scenario(_scenario_name: str, _fallback_end_uses: list) -> list:
                ...
                try:
                    ...
                    if isinstance(_df, pd.DataFrame) and "Month" in _df.columns:
                        return _df.melt(id_vars="Month", var_name="End_Use", value_name="kWh")[
                            "End_Use"].unique().tolist()
                except Exception:
                    pass
            ...
            if st.button("New", use_container_width=True, key="scenario_btn_new", disabled=IS_VIEWER_MODE,
                         help=_viewer_widget_help()):
                _save_current_scenario_payload(current_active)
                base_name = "Scenario"
                n = 1
                ...
                try:
                    set_scenario_energy_balance_override(
                        new_name,
                        get_energy_balance_df(file_bytes, uploaded_file.name, scenario_name=current_active,
                                              apply_master_filter=False),
                    )
                except Exception:
                    pass
                try:
                    set_scenario_loads_balance_override(
                        new_name,
                        get_loads_balance_df(file_bytes, uploaded_file.name, scenario_name=current_active,
                                             apply_master_filter=False),
                    )
                except Exception:
                    pass
            ...
            if st.button("Duplicate", use_container_width=True, key="scenario_btn_duplicate", disabled=IS_VIEWER_MODE,
                         help=_viewer_widget_help()):
                _save_current_scenario_payload(current_active)
                ...
                try:
                    cmap_sc = st.session_state.get("color_map_scenarios", {})
                    cmap_sc[new_name] = cmap_sc.get(current_active, SCENARIO_COLOR_PALETTE[
                        (len(scenarios) - 1) % len(SCENARIO_COLOR_PALETTE)])
                    st.session_state["color_map_scenarios"] = cmap_sc
                except Exception:
                    pass
                st.session_state["scenarios"] = scenarios
            ...
            _scenario_move_up_disabled = IS_VIEWER_MODE or current_active not in scenario_names or scenario_names.index(
                current_active) <= 0
            if st.button(
                    "Move Up",
                    ...
                    help=_viewer_widget_help(
                        "Move the active scenario one position up. Exported numbered Energy/Loads sheets follow this order."),
            ):
                _save_current_scenario_payload(current_active)
                ...
                if current_active in _old_order:
                    ...
                    if _idx_move > 0:
                        ...
                        _new_order[_idx_move - 1], _new_order[_idx_move] = _new_order[_idx_move], _new_order[
                            _idx_move - 1]
            ...
            rename_to = st.text_input("Rename to", value="", key="scenario_rename_to", disabled=IS_VIEWER_MODE,
                                      help=_viewer_widget_help())
            if st.button("Rename", use_container_width=True, key="scenario_btn_rename", disabled=IS_VIEWER_MODE,
                         help=_viewer_widget_help()):
            ...
            if st.button("Delete", use_container_width=True, key="scenario_btn_delete", disabled=IS_VIEWER_MODE,
                         help=_viewer_widget_help()):
                if len(scenarios) > 1 and current_active in scenarios:
                    ...
                    st.rerun()
            st.caption(
                "Scenarios store CO₂ factors, tariffs, source mapping, efficiency factors, On-site generation settings, CRREM measures, scenario-specific LCC investment measures, and optional scenario-specific Energy_Balance / Loads_Balance raw data. Numbered Excel sheets are mapped by scenario order.")
        ...
        with st.sidebar.expander("Emission Factors"):
            ...
            st.checkbox(
                ...
                help=_viewer_widget_help(
                    "If active, electricity uses the selected country's CRREM grid emission factor for the project year and follows the CRREM grid decarbonization pathway. Other sources still use user-entered initial factors."),
                disabled=IS_VIEWER_MODE,
            )
            ...
            if st.session_state.get("crrem_use_standard_electricity_ef", False):
                if _crrem_elec_ef_preview is not None:
                    ...
                    st.warning(
                        "CRREM electricity factor could not be loaded for the selected country/year. The user-entered electricity factor is used instead.")

            _elec_default = float(_crrem_elec_ef_preview) if (st.session_state.get("crrem_use_standard_electricity_ef",
                                                                                   False) and _crrem_elec_ef_preview is not None) else float(
                def_f.get("Electricity", 0.300))
            ...
            co2_Emissions_Green_Electricity = numeric_input("CO2 Factor Green Electricity",
                                                            ...
                                                            max_value=1.0, fmt="{:.5f}", disabled=IS_VIEWER_MODE,
                                                            help=_viewer_widget_help())
            ...
            co2_emissions_gas = numeric_input("CO2 Factor Gas", float(def_f.get("Gas", 0.180)), key="co2_emissions_gas",
                                              min_value=0.0, max_value=1.0, fmt="{:.5f}", disabled=IS_VIEWER_MODE,
                                              help=_viewer_widget_help())
            co2_emissions_biomass = numeric_input("CO2 Factor Biomass", float(def_f.get("Biomass", 0.000)),
                                                  key="co2_emissions_biomass",
                                                  min_value=0.0, max_value=5.0, fmt="{:.5f}", disabled=IS_VIEWER_MODE,
                                                  help=_viewer_widget_help())
        ...
        with st.sidebar.expander("Energy Tariffs"):
            st.caption(
                "Assign source tariffs. Optionally add an annual peak-demand charge (per kW) to the consumption tariff (per kWh).")
        ...
        with st.sidebar.expander("Color Settings", expanded=False):
            ...
            if "color_map_enduse" not in st.session_state or not isinstance(st.session_state.get("color_map_enduse"),
                                                                            dict):
                st.session_state["color_map_enduse"] = dict(DEFAULT_COLOR_MAP)
            if "color_map_sources" not in st.session_state or not isinstance(st.session_state.get("color_map_sources"),
                                                                             dict):
                st.session_state["color_map_sources"] = dict(DEFAULT_COLOR_MAP_SOURCES)
            if "color_map_loads" not in st.session_state or not isinstance(st.session_state.get("color_map_loads"),
                                                                           dict):
                st.session_state["color_map_loads"] = dict(DEFAULT_COLOR_MAP_LOADS)
            if "color_map_scenarios" not in st.session_state or not isinstance(
                    st.session_state.get("color_map_scenarios"), dict):
                st.session_state["color_map_scenarios"] = default_scenario_color_map(
                    list(st.session_state.get("scenarios", {}).keys()))
            ...
            try:
                ...
                _load_cols = [c for c in _df_loads_sidebar.columns if
                              c not in ["hoy", "doy", "day", "month", "weekday", "hour"]]
            except Exception:
                _load_cols = []
            ...
            for _ld in _load_cols:
                if _ld not in st.session_state["color_map_loads"]:
                    ...
                    st.session_state["color_map_loads"][_ld] = st.session_state["color_map_enduse"].get(_ld, _rand_hex(
                        f"load::{_ld}"))
            ...
            for _i_sc_col, _sc_col in enumerate(_scenario_names_for_colors):
                if _sc_col not in st.session_state["color_map_scenarios"]:
                    st.session_state["color_map_scenarios"][_sc_col] = SCENARIO_COLOR_PALETTE[
                        _i_sc_col % len(SCENARIO_COLOR_PALETTE)]
            ...
            st.caption(
                "Customize colors for End Uses, Energy Sources, and Loads. These settings are saved with the project.")
            ...
            for _eu in end_uses:
                ...
                _new = st.color_picker(ui_name(str(_eu)), value=_val, key=_key, disabled=IS_VIEWER_MODE,
                                       help=_viewer_widget_help())
            ...
            for _src in _sources_in_use:
                ...
                _new = st.color_picker(str(_src), value=_val, key=_key, disabled=IS_VIEWER_MODE,
                                       help=_viewer_widget_help())
            ...
            if _load_cols:
                for _ld in _load_cols:
                    ...
                    _new = st.color_picker(ui_name(str(_ld)), value=_val, key=_key, disabled=IS_VIEWER_MODE,
                                           help=_viewer_widget_help())
            ...
            if _scenario_names_for_colors:
                for _sc in _scenario_names_for_colors:
                    ...
                    _new = st.color_picker(str(_sc), value=_val, key=_key, disabled=IS_VIEWER_MODE,
                                           help=_viewer_widget_help())
        ...
        color_map_scenarios = st.session_state.get("color_map_scenarios", default_scenario_color_map(
            list(st.session_state.get("scenarios", {}).keys())))
        ...
        with st.sidebar:
            if (not IS_VIEWER_MODE) and st.button("Save Project", use_container_width=True):
                ...
                if "scenarios" in st.session_state:
                    ...
                    try:
                        ...
                        lcc_investments_df = build_lcc_investments_sheet(st.session_state.get("scenarios", {}),
                                                                         end_uses)
                    except Exception:
                        lcc_global_df = None
                        lcc_investments_df = None
                ...
                colors_df = build_color_settings_df(
                    ...
                    st.session_state.get("color_map_scenarios", default_scenario_color_map(
                        list(st.session_state.get("scenarios", {}).keys()))),
                )
            ...
            if (not IS_VIEWER_MODE) and st.button("Generate Report", use_container_width=True,
                                                  key="btn_generate_report"):
                try:
                    with st.spinner("Generating A4 PDF report for the active scenario..."):
                        if "scenarios" in st.session_state and st.session_state.get("active_scenario") in \
                                st.session_state["scenarios"]:
                            st.session_state["scenarios"][
                                st.session_state["active_scenario"]] = capture_scenario_from_widgets(end_uses)
                            if st.session_state.get("_lcc_global_initialized"):
                        ...
                        st.session_state[
                            "_generated_report_name"] = f"{_report_sanitize_filename(st.session_state.get('project_name', 'BPVis_Project'))}_{_report_sanitize_filename(st.session_state.get('active_scenario', 'Scenario'))}_Report_v2_4_7.pdf"
            ...
            if (not IS_VIEWER_MODE) and st.session_state.get("_generated_report_pdf"):
                st.download_button(
                    ...
                    file_name=st.session_state.get("_generated_report_name", "BPVis_Report_v2_4_7.pdf"),
        ...
        pv_value = totals_indexed.loc[
            "On-site_Generation", "kWh_per_m2"] if "On-site_Generation" in totals_indexed.index else 0.0
...
with tab_model_qa:
    if (not IS_VIEWER_MODE) and uploaded_file:
        ...
        with st.form("model_inputs_qa_add_object_form", clear_on_submit=False):
            ...
            with add_col2:
                with st.expander("Add envelope component", expanded=False):
                    new_env_type = st.selectbox("Construction/component type", MODEL_INPUT_ENVELOPE_COMPONENT_TYPES,
                                                key="mi_new_envelope_type")
                    new_env_name = st.text_input("Component name", value="New component", key="mi_new_envelope_name")
                    new_env_scope_label = st.selectbox("Scope", MODEL_INPUT_OBJECT_SCOPE_OPTIONS, index=1,
                                                       key="mi_new_envelope_scope")
            ...
            with add_col3:
                with st.expander("Add system", expanded=False):
                    ...
                    new_system_scope_label = st.selectbox("Scope", MODEL_INPUT_OBJECT_SCOPE_OPTIONS, index=1,
                                                          key="mi_new_system_scope")
        ...
        mi_full = sanitize_model_inputs_qa_df(st.session_state.get("model_inputs_qa_df")).reset_index().rename(
            columns={"index": "_orig_index"})
        mask_active = mi_full["Scenario"].astype(str).eq(MODEL_INPUT_GLOBAL_SCENARIO) | mi_full["Scenario"].astype(
            str).eq(str(active_selected))
        mi_edit = mi_full.loc[mask_active].copy()
        mi_keep_other = mi_full.loc[~mask_active].drop(columns=["_orig_index"]).copy()

        _editor_key_token = hashlib.md5(
            str(st.session_state.get(_RAW_TOKEN_KEY, "model_inputs_structured")).encode("utf-8")).hexdigest()[:8]
        edited_rows = []
        ...
        def _value_widget(row, key_prefix):
            ...
            if p_low == "thermal mass":
                ...
                selected_default = option_lookup.get(current_l,
                                                     "Other / Custom thermal capacity" if current_value else "")
            ...
            if p_low in ["heating delivery", "cooling delivery"]:
                ...
                current_value = value.strip()
                selected_default = current_value if current_value in delivery_options else (
                    "Other / Custom" if current_value else "")
                selected = st.selectbox(
                    "Value",
                    delivery_options,
            ...
            if "energy source" in p_low:
                ...
                val = value if value in opts else ""
                return st.selectbox("Value", opts, index=opts.index(val), key=f"{key_prefix}_value")
            if u_low.strip() == "yes/no" or p_low.endswith("permitted") or p_low in ["demand controlled ventilation",
                                                                                     "natural ventilation permitted"]:
        ...
        def _render_model_input_item(item, source_df):
            ...
            with st.expander(_object_label, expanded=False):
                ...
                with st.expander("Object ID", expanded=False):
                    item_delete_key = _safe_model_input_key("mi", _editor_key_token, "delete_item", scenario_i,
                                                            category_i, item_type_i, item_name_i)
                    item_duplicate_key = _safe_model_input_key("mi", _editor_key_token, "duplicate_item", scenario_i,
                                                               category_i, item_type_i, item_name_i)
                    item_rename_key = _safe_model_input_key("mi", _editor_key_token, "rename_item", scenario_i,
                                                            category_i, item_type_i, item_name_i)
                    item_duplicate_name_key = _safe_model_input_key("mi", _editor_key_token, "duplicate_item_name",
                                                                    scenario_i, category_i, item_type_i, item_name_i)
                    item_rename_name_key = _safe_model_input_key("mi", _editor_key_token, "rename_item_name",
                                                                 scenario_i, category_i, item_type_i, item_name_i)
                    ...
                    if not id_rows_item.empty:
                        ...
                        id_key_prefix = _safe_model_input_key("mi", _editor_key_token, id_orig_idx, scenario_i,
                                                              category_i, item_type_i, item_name_i, "ID")
                        st.caption("Object identifier used for traceability in saved projects and reports.")
                        id_c1, id_c2, id_c3 = st.columns([1.6, 1.2, 2.4])
                        with id_c1:
                            object_id_value = st.text_input("ID", value=str(id_row.get("Value", "")),
                                                            key=f"{id_key_prefix}_value")
                        with id_c2:
                            ...
                            object_id_source = st.selectbox("Source Type", MODEL_INPUT_SOURCE_TYPES,
                                                            index=MODEL_INPUT_SOURCE_TYPES.index(current_id_source),
                                                            key=f"{id_key_prefix}_source")
                        with id_c3:
                            object_id_ref = st.text_input("Source document / reference", value=str(
                                id_row.get("Source Document / Reference", "Model Inputs QA")),
                                                          key=f"{id_key_prefix}_source_ref")
                        object_id_notes = st.text_input("ID notes", value=str(id_row.get("Notes", "")),
                                                        key=f"{id_key_prefix}_notes")
                ...
                with st.expander("Add custom parameter", expanded=False):
                    custom_key_prefix = _safe_model_input_key("mi", _editor_key_token, "custom", scenario_i, category_i,
                                                              item_type_i, item_name_i)
                    custom_param_name = st.text_input("Custom parameter name", value="",
                                                      key=f"{custom_key_prefix}_name")
                    ...
                    if custom_param_unit_choice == "Other / Custom":
                        custom_param_unit = st.text_input("Custom unit", value="",
                                                          key=f"{custom_key_prefix}_unit_custom").strip() or "Other / Custom"
                    else:
                    ...
                    custom_param_ref = st.text_input("Source document / reference", value="",
                                                     key=f"{custom_key_prefix}_source_ref")
                ...
                for _, row in rows_item.iterrows():
                    ...
                    key_prefix = _safe_model_input_key("mi", _editor_key_token, orig_idx, scenario_i, category_i,
                                                       item_type_i, item_name_i, parameter)
                    if str(row.get("Source Type", "")) == "Assumption":
                        st.markdown(
                            "<div style='background-color:#fff3cd;padding:4px 8px;border-radius:4px;margin-top:6px;'><b>Assumption-tagged input</b></div>",
                    ...
                    with c3:
                        ...
                        _unit_default = _unit_current if _unit_current in _unit_options else (
                            _unit_options[0] if _unit_options else "-")
                        ...
                        if _unit_selected == "Other / Custom":
                            ...
                            unit = _unit_selected
                    with c4:
                        required = st.checkbox("Required", value=bool(row.get("Required", False)),
                    ...
                    with c6:
                        ...
                        source_type = st.selectbox("Source Type", MODEL_INPUT_SOURCE_TYPES,
                                                   index=MODEL_INPUT_SOURCE_TYPES.index(current_source),
                                                   key=f"{key_prefix}_source")
                    with c7:
                        source_ref = st.text_input("Source document / reference",
                                                   value=str(row.get("Source Document / Reference", "")),
                                                   key=f"{key_prefix}_source_ref")
                    with c8:
                        reference = st.text_input("Reference / target", value=str(row.get("Reference / Target", "")),
                    ...
                    try:
                        min_check = float(str(min_check_txt).replace(",", ".")) if str(
                            min_check_txt).strip() != "" else np.nan
                    except Exception:
                        min_check = np.nan
                    try:
                        max_check = float(str(max_check_txt).replace(",", ".")) if str(
                            max_check_txt).strip() != "" else np.nan
                    except Exception:
                        max_check = np.nan
        ...
        with st.form("model_inputs_qa_structured_form", clear_on_submit=False):
            for cat in MODEL_INPUT_CATEGORIES:
                ...
                if cat == "General Model Setup":
                    cat_df = mi_edit.loc[
                        mi_edit["Category"].astype(str).isin(["General Model Setup", "Room Types"])].copy()
                else:
                ...
                with st.expander(cat, expanded=False):
                    if cat == "General Model Setup":
                        st.caption(
                            "This section contains global simulation setup inputs and room-type definitions. Room types are global by default, but can also be created for the active scenario only.")
                    item_df_keys = cat_df[
                        ["Scenario", "Scope", "Category", "Item Type", "Item Name"]].drop_duplicates().reset_index(
                        drop=True)
                    if cat == "General Model Setup":
                        general_item_keys = item_df_keys.loc[
                            item_df_keys["Category"].astype(str).eq("General Model Setup")]
                        ...
                        st.caption(
                            "Envelope components are grouped by construction/component type. Open a component-type group first, then open the specific construction/object to edit its parameters.")
                        try:
                            _item_types_present = item_df_keys["Item Type"].dropna().astype(str).tolist()
                        except Exception:
                            _item_types_present = []
                        _ordered_env_types = [t for t in MODEL_INPUT_ENVELOPE_COMPONENT_TYPES if
                                              t in set(_item_types_present)]
                        _extra_env_types = sorted(
                            [t for t in set(_item_types_present) if t not in set(_ordered_env_types)])
                        for _env_type in _ordered_env_types + _extra_env_types:
                            _env_type_keys = item_df_keys.loc[
                                item_df_keys["Item Type"].astype(str).eq(str(_env_type))].copy()
        ...
        def _model_input_filtered_rows_current_form():
            ...
            for rec in list(edited_rows) + list(custom_rows_to_add):
                item_key = (str(rec.get("Scenario")), str(rec.get("Category")), str(rec.get("Item Type")),
                            str(rec.get("Item Name")))
        ...
        if duplicate_item_requests:
            ...
            for dup_key, requested_name in duplicate_item_requests:
                ...
                if dup_key in deleted_item_keys:
                    continue
                new_item_name = _model_input_next_copy_name(list(rows_filtered) + list(duplicate_rows), dup_key,
                                                            requested_name)
                for rec in rows_filtered:
                    rec_key = (str(rec.get("Scenario")), str(rec.get("Category")), str(rec.get("Item Type")),
                               str(rec.get("Item Name")))
                    if rec_key != dup_key:
                        continue
                    new_rec = dict(rec)
                    ...
                    if str(new_rec.get("Parameter", "")) == "ID":
                        new_rec["Value"] = _model_input_object_id(str(new_rec.get("Category", "")),
                                                                  str(new_rec.get("Item Type", "")), new_item_name,
                                                                  str(new_rec.get("Scenario", "")))
...
The QA is intended as traceability and model-quality control. It does not replace formal ASHRAE 90.1, LEED, local-code, or certification documentation.
            ...
            qa_eval = evaluate_model_inputs_qa_df(
                model_inputs_df_for_scenario(st.session_state.get("model_inputs_qa_df"), active_selected))
            qa_cols = [
                "Scope", "Category", "Item Type", "Item Name", "Parameter", "Value", "Unit", "Required", "Source Type",
                "Source Document / Reference", "Reference / Target", "Usual Min", "Usual Max", "Range Justification",
                "QA Status", "QA Message", "Notes"
            ]
            try:
                st.dataframe(
        ...
        if crrem is None:
            ...
            if project_area_val <= 0:
                ...
                emissions_series = pd.Series({
                    int(y): sum(
                        float(kwh) * float(_source_emission_factor_at_year(
                            str(src), int(y), int(project_year_val), base_factors, ef_grid, crrem_ef_settings_active,
                            None
                        ))
                        for src, kwh in annual_kwh_by_source.items()
                    )
                ...
                with st.expander("Decarbonization Path Analysis", expanded=False):
                    ...
                    try:
                        param_specs["PV_Generation → PV Annual Production (kWh/a)"] = param_specs[
                            "On-site_Generation → Annual Production (kWh/a)"]
                    except Exception:
                        pass
                    ...
                    with st.expander("Measures (scenario-specific)", expanded=False):
                        ...
                        if "crrem_measures_draft_df" not in st.session_state or not isinstance(
                            ...
                            st.session_state["crrem_measures_draft_df"] = st.session_state["crrem_measures_df"].copy(
                                deep=True)
                        ...
                        def _ef_at_year(src: str, year: int, inclusive: bool = True) -> float:
                            ...
                            for ym, vm in ef_measures.get(str(src), []):
                                try:
                                    ...
                                    pass
                            return _source_emission_factor_at_year(
                                str(src), int(year), int(project_year_val), base_factors, ef_grid,
                                crrem_ef_settings_active, _ef_list
                            )
                        ...
                        with st.expander("Additional CRREM diagrams — With measures", expanded=False):
                            ...
                            try:
                                annual_emission_savings_t = (
                                        (carbon_asset.reindex(years_avail).astype(float) - carbon_meas_s.reindex(
                                            years_avail).astype(float))
                                ...
                                annual_end_energy_savings_mwh = (
                                        (eui_asset_series.reindex(years_avail).astype(float) - eui_meas_s.reindex(
                                            years_avail).astype(float))
                                        * float(project_area_val)
                                        / 1000.0
                                )
                                ...
                                with savings_col1:
                                    ...
                                    st_plotly_chart(fig_sav_ann, use_container_width=True,
                                                    key="crrem_annual_emission_savings")
                                    ...
                                    st_plotly_chart(fig_sav_cum, use_container_width=True,
                                                    key="crrem_cumulative_emission_savings")
                                ...
                                with savings_col2:
                                    ...
                                    fig_energy_sav_ann.add_hline(y=0, line_width=2, line_dash="dash",
                                                                 line_color="black")
                                    ...
                                    st_plotly_chart(fig_energy_sav_ann, use_container_width=True,
                                                    key="crrem_annual_end_energy_savings")
                                    ...
                                    fig_energy_sav_cum.add_hline(y=0, line_width=2, line_dash="dash",
                                                                 line_color="black")
                                    ...
                                    st_plotly_chart(fig_energy_sav_cum, use_container_width=True,
                                                    key="crrem_cumulative_end_energy_savings")
                                ...
                                st.warning(
                                    "CRREM savings charts could not be generated for the current measures setup.")
...
with tab_lcc:
    if uploaded_file:
        ...
        if "lcc_investments_df" not in st.session_state or not isinstance(st.session_state.get("lcc_investments_df"),
                                                                          pd.DataFrame):
        ...
        with st.expander("LCC-Analysis", expanded=True):
            ...
            ref_default_lcc = st.session_state.get(ref_key_lcc,
                                                   lcc_global_committed.get("payback_reference_scenario", ""))
            ...
            if "lcc_investments_draft_df" not in st.session_state or not isinstance(
                    st.session_state.get("lcc_investments_draft_df"), pd.DataFrame):
            ...
            if st.session_state.get("_lcc_flash") == "updated":
                st.success(
                    "LCC inputs updated. Global LCC parameters were applied to all scenarios; investment measures were applied to the active scenario.")
            ...
            with st.form("lcc_analysis_form", clear_on_submit=False):
                ...
                st.multiselect(
                    ...
                    default=st.session_state.get(_lcc_global_draft_key("selected_operational_end_uses"),
                                                 _lcc_default_selected_enduses(valid_enduses_lcc)),
                    format_func=ui_name,
                    key=_lcc_global_draft_key("selected_operational_end_uses"),
                    help=_viewer_widget_help(
                        "Only the selected End Uses are included in the operational energy-cost part of the LCC analysis."),
                    disabled=IS_VIEWER_MODE,
                )
                ...
                st.selectbox(
                    ...
                    index=ref_options_lcc.index(st.session_state.get(ref_key_lcc, "")) if st.session_state.get(
                        ref_key_lcc, "") in ref_options_lcc else 0,
                ...
                draft_analysis_period_for_editor = max(
                    1,
                    _to_int_lcc(
                        st.session_state.get(_lcc_global_draft_key("analysis_period"),
                                             lcc_global_committed.get("analysis_period", 30)),
        ...
        if active_lcc_cashflow.empty:
            st.info(
                "No LCC cash flows available. Add investment data and/or select operational End Uses in the LCC input expander.")
        else:
            ...
            for _i_sc, _sc_name in enumerate(_lcc_scenario_order):
                _lcc_scenario_colors[str(_sc_name)] = str(
                    _lcc_saved_scenario_colors.get(str(_sc_name),
                                                   SCENARIO_COLOR_PALETTE[_i_sc % len(SCENARIO_COLOR_PALETTE)])
                )
            ...
            with c3:
                ...
                _lcc_show_savings = st.checkbox(
                    "Show savings",
                    value=True,
                    key="lcc_cumulative_show_savings",
                    help="Show the final-year nominal and/or discounted saving versus the selected reference scenario.",
                )
                ...
                if not ref_annual_totals.empty:
                    ...
                    # Final-year cumulative savings versus the selected reference scenario.
                    # Saving = reference cumulative cost - active cumulative cost.
                    # The optional annotations are skipped entirely when the user disables them.
                    if _lcc_show_savings:
                        try:
                            _active_years = pd.to_numeric(annual_totals["Year"], errors="coerce").dropna()
                            _ref_years = pd.to_numeric(ref_annual_totals["Year"], errors="coerce").dropna()
                            if not _active_years.empty and not _ref_years.empty:
                                _lcc_saving_year = int(min(float(_active_years.max()), float(_ref_years.max())))
                                _active_final = annual_totals.loc[
                                    pd.to_numeric(annual_totals["Year"], errors="coerce") == _lcc_saving_year
                                    ]
                                _ref_final = ref_annual_totals.loc[
                                    pd.to_numeric(ref_annual_totals["Year"], errors="coerce") == _lcc_saving_year
                                    ]
                                if not _active_final.empty and not _ref_final.empty:
                                    _active_nominal_final = float(_active_final["Cumulative Nominal Cost"].iloc[-1])
                                    _ref_nominal_final = float(_ref_final["Cumulative Nominal Cost"].iloc[-1])
                                    _active_discounted_final = float(
                                        _active_final["Cumulative Discounted Cost"].iloc[-1])
                                    _ref_discounted_final = float(_ref_final["Cumulative Discounted Cost"].iloc[-1])
                                    _nominal_saving = _ref_nominal_final - _active_nominal_final
                                    _discounted_saving = _ref_discounted_final - _active_discounted_final


                                    def _lcc_saving_text(_label, _value):
                                        _sign = "−" if _value < 0 else ""
                                        return f"<b>{_label}</b><br>{_sign}{currency_lcc} {abs(_value):,.0f}"


                                    _saving_x_nominal = _lcc_saving_year + 0.55
                                    _saving_x_discounted = _lcc_saving_year + 1.05

                                    if _lcc_show_nominal:
                                        _nom_y0, _nom_y1 = sorted([_active_nominal_final, _ref_nominal_final])
                                        fig_lcc_cum.add_trace(go.Scatter(
                                            x=[_saving_x_nominal, _saving_x_nominal],
                                            y=[_nom_y0, _nom_y1],
                                            mode="lines+markers",
                                        ...
                                        fig_lcc_cum.add_annotation(
                                            x=_saving_x_nominal + 0.12,
                                            y=(_nom_y0 + _nom_y1) / 2.0,
                                            text=_lcc_saving_text("Nominal", _nominal_saving),
                                            showarrow=False,
                                            xanchor="left",
                                            align="left",
                                    ...
                                    if _lcc_show_discounted:
                                        _disc_y0, _disc_y1 = sorted([_active_discounted_final, _ref_discounted_final])
                                        fig_lcc_cum.add_trace(go.Scatter(
                                            x=[_saving_x_discounted, _saving_x_discounted],
                                            y=[_disc_y0, _disc_y1],
                                            mode="lines+markers",
                                        ...
                                        fig_lcc_cum.add_annotation(
                                            x=_saving_x_discounted + 0.12,
                                            y=(_disc_y0 + _disc_y1) / 2.0,
                                            text=_lcc_saving_text("Discounted", _discounted_saving),
                                            showarrow=False,
                                            xanchor="left",
                                            align="left",
                                            bgcolor="rgba(255,255,255,0)",
                                    ...
                                    _lcc_min_year = float(min(_active_years.min(), _ref_years.min()))
                                    fig_lcc_cum.update_xaxes(
                                        range=[_lcc_min_year - 0.5, _lcc_saving_year + 3.0]
                                    )
                        except Exception:
                            pass
                fig_lcc_cum.update_layout(
                    ...
                    legend_title="",
                    legend=dict(orientation="h", yanchor="top", y=-0.18, xanchor="center", x=0.5),
                    margin=dict(l=40, r=45, t=50, b=80),
                )
                fig_lcc_cum.update_yaxes(rangemode="tozero")
                st_plotly_chart(fig_lcc_cum, use_container_width=True, key="lcc_cumulative")
                if not ref_annual_totals.empty and _lcc_show_savings:
                    st.caption(
                        f"Final-year savings are calculated as {ref_scenario_lcc} cumulative cost minus "
                        f"{active_selected} cumulative cost. Negative values indicate an additional cost for the active scenario."
                    )
            ...
            with c4:
                ...
                if not pie_type.empty:
                    ...
                    fig_type_pie.update_traces(texttemplate="%{value:.1f}<br>%{percent}", textinfo="none",
            ...
            with b1:
                ...
                if not pie_enduse.empty:
                    ...
                    fig_enduse_pie.update_traces(texttemplate="%{value:.1f}<br>%{percent}", textinfo="none",
            ...
            with b2:
                ...
                source_breakdown = active_lcc_cashflow.loc[active_lcc_cashflow["Cost Type"] == "Energy"].groupby(
                    "Energy_Source", as_index=False).agg(
                    Nominal_Cost=("Nominal Cost", "sum"),
                    Discounted_Cost=("Discounted Cost", "sum"),
                )
                ...
                if not pie_source.empty:
                    ...
                    fig_source_pie.update_traces(texttemplate="%{value:.1f}<br>%{percent}", textinfo="none",
                                                 textfont_size=18, textfont_color="white")
    ...
    if uploaded_file:
        ...
        if not scenarios:
            ...
            for i, s in enumerate(scenario_order):
                scenario_color_map[str(s)] = str(
                    _saved_scenario_colors.get(str(s), SCENARIO_COLOR_PALETTE[i % len(SCENARIO_COLOR_PALETTE)]))
            ...
            if SCENARIO_COMPARISON_PERIOD_KEY not in st.session_state:
                st.session_state[SCENARIO_COMPARISON_PERIOD_KEY] = int(
                    (preloaded or {}).get("scenario_comparison_period", 50))
            if SCENARIO_COMPARISON_APPLY_LCC_FILTER_KEY not in st.session_state:
                st.session_state[SCENARIO_COMPARISON_APPLY_LCC_FILTER_KEY] = bool(
                    (preloaded or {}).get("scenario_comparison_apply_lcc_filter", True))
            ...
            scenario_order = [str(s) for s in _comparison_scenario_options if
                              str(s) in set(_comparison_scenario_selected)]
            ...
            df_cmp_display = df_cmp_display.loc[
                df_cmp_display["Scenario"].astype(str).isin(_comparison_scenario_set)].copy()
            ...
            emissions_use_rows = [r for r in emissions_use_rows if
                                  str(r.get("Scenario", "")) in _comparison_scenario_set]
            ...
            try:
                ...
                for _sc_tmp in scenario_order:
                    try:
                        ...
                        pass
                _radar_enduses_all = list(
                    dict.fromkeys([_canon_enduse_name(str(u)) for u in _radar_enduses_all if str(u).strip()]))
                ...
                with _ctrl_col:
                    if _scenario_order_str:
                        _radar_ref_scenario = st.selectbox(
                            ...
                            index=_scenario_order_str.index(
                                _radar_ref_default) if _radar_ref_default in _scenario_order_str else 0,
                ...
                if (not _radar_absolute_df.empty and _radar_absolute_df["Radial"].notna().any()):
                    ...
                    with rcol1:
                        if not _radar_improvement_df.empty and _radar_improvement_df["Radial"].notna().any():
                            ...
                            _radar_render_signature = hashlib.md5(
                                ("|".join(scenario_order) + "||" + "|".join(
                                    sorted(_comparison_kpi_selected_labels))).encode("utf-8")
                            ).hexdigest()[:10]
                            st_plotly_chart(
                    ...
                    st_plotly_chart(fig_scenario_kpi_scatter, use_container_width=True,
                                    key="scenario_kpi_scatter_summary")
                    st.caption(
                    ...
                    with st.expander("Scenario Delta Analysis", expanded=True):
                        ...
                        if _delta_ref_default not in _delta_scenario_options:
                            _delta_ref_default = str(
                                st.session_state.get("scenario_radar_reference_scenario", "") or "")
                        if _delta_ref_default not in _delta_scenario_options:
                            _delta_ref_default = str(active_selected) if str(
                                active_selected) in _delta_scenario_options else (
                                _delta_scenario_options[0] if _delta_scenario_options else ""
                            )
                        if _delta_ref_default and st.session_state.get(
                                _delta_ref_widget_key) not in _delta_scenario_options:
                        ...
                        if _delta_scenario_options:
                            ...
                            with _delta_ctrl_col:
                                _delta_ref_scenario = st.selectbox(
                                    ...
                                    index=_delta_scenario_options.index(
                                        _delta_ref_default) if _delta_ref_default in _delta_scenario_options else 0,
                            ...
                            if _delta_long_df.empty:
                                st.info(
                                    "At least two scenarios with valid KPI values are required for scenario delta analysis.")
                            else:
                                ...
                                with _dcol1:
                                    ...
                                    st_plotly_chart(_fig_delta_pct, use_container_width=True,
                                                    key="scenario_delta_improvement_pct")
                                with _dcol2:
                                    ...
                                    if _fig_delta_mac.data:
                                        st_plotly_chart(_fig_delta_mac, use_container_width=True,
                                                        key="scenario_delta_mac")
                                    else:
                                        st.info(
                                            "Marginal abatement cost is available only when life-cycle emissions are reduced compared with the reference.")

                                _delta_display = _delta_long_df.copy()
                                for _c in ["Reference Value", "Scenario Value", "Delta (Scenario - Reference)",
                                           "Improvement (Reference - Scenario)", "Improvement vs Reference (%)"]:
                                    if _c in _delta_display.columns:
                                        _delta_display[_c] = pd.to_numeric(_delta_display[_c], errors="coerce")
                                _delta_display["Reference Value"] = _delta_display["Reference Value"].map(
                                    lambda v: "n/a" if pd.isna(v) else f"{float(v):,.2f}")
                                _delta_display["Scenario Value"] = _delta_display["Scenario Value"].map(
                                    lambda v: "n/a" if pd.isna(v) else f"{float(v):,.2f}")
                                _delta_display["Delta"] = _delta_display["Delta (Scenario - Reference)"].map(
                                    lambda v: "n/a" if pd.isna(v) else f"{float(v):,.2f}")
                                _delta_display["Improvement"] = _delta_display[
                                    "Improvement (Reference - Scenario)"].map(
                                    lambda v: "n/a" if pd.isna(v) else f"{float(v):,.2f}")
                                _delta_display["Improvement %"] = _delta_display["Improvement vs Reference (%)"].map(
                                ...
                                if isinstance(_delta_summary_df, pd.DataFrame) and not _delta_summary_df.empty:
                                    ...
                                    for _c in _summary_display.columns:
                                        if _c not in ["Reference Scenario", "Scenario"]:
                                            _summary_display[_c] = pd.to_numeric(_summary_display[_c],
                                                                                 errors="coerce").map(
                                                lambda v: "n/a" if pd.isna(v) else f"{float(v):,.2f}"
                                            )
            ...
            with st.expander("Life Cycle Comparission", expanded=True):
                ...
                with _lc_ctrl2:
                    ...
                    st.session_state[_scenario_crrem_toggle_key] = bool(
                        st.session_state.get(CRREM_SHOW_BASELINE_STATE_KEY, True))
                    st.checkbox(
                        "Show CRREM baseline / pathway lines",
                ...
                try:
                    ...
                    pass
                all_enduses_lcc_cmp = list(
                    dict.fromkeys([_canon_enduse_name(str(u)) for u in all_enduses_lcc_cmp if str(u).strip()]))
                ...
                if not all_enduses_lcc_cmp:
                    ...
                    for _idx_sc, _sc_name in enumerate(scenario_order):
                        ...
                        _color_sc = scenario_color_map.get(_sc_name, SCENARIO_COLOR_PALETTE[
                            _idx_sc % len(SCENARIO_COLOR_PALETTE)])
                        try:
                            _df_energy_sc = get_energy_balance_df(file_bytes, uploaded_file.name,
                        ...
                        lcc_summary_rows.append({
                            "Scenario": _sc_name,
                            f"Cumulative Nominal LCC ({_curr})": float(_cum_nominal_sc.iloc[-1]) if len(
                                _cum_nominal_sc) else 0.0,
                            f"Cumulative Discounted LCC ({_curr})": float(_cum_discounted_sc.iloc[-1]) if len(
                                _cum_discounted_sc) else 0.0,
                        })
                    ...
                    try:
                        ...
                        if _crrem_cmp is not None and _area and float(_area) > 0:
                            ...
                            _use_options_cmp = _pt_df_cmp["app_use"].dropna().astype(
                                str).tolist() if "app_use" in _pt_df_cmp.columns else []
                            ...
                            if _crrem_use_cmp not in _use_options_cmp:
                                try:
                                    ...
                                    _crrem_use_cmp = str((scenarios.get(_active_sc_cmp, {}) or {}).get("crrem_use_type",
                                                                                                       _crrem_use_cmp) or _crrem_use_cmp)
                                except Exception:
                                    pass
                            if _crrem_use_cmp not in _use_options_cmp:
                                _crrem_use_cmp = "Office" if "Office" in _use_options_cmp else (
                                    _use_options_cmp[0] if _use_options_cmp else "")

                            _pc_cmp = _crrem_cmp.get("pathways_carbon", pd.DataFrame()).copy()
                            if (not _pc_cmp.empty) and {"target", "year", "property_type_code",
                                                        "kgco2e_per_m2_yr"}.issubset(_pc_cmp.columns):
                                ...
                                if _years_crrem_cmp and _crrem_use_cmp and _crrem_use_cmp != "Mixed Use":
                                    _code_row_cmp = _pt_df_cmp.loc[
                                        _pt_df_cmp["app_use"].astype(str) == str(_crrem_use_cmp)]
                                    if not _code_row_cmp.empty:
                                        _p_code_cmp = str(_code_row_cmp.iloc[0]["crrem_code"])
                                        if _p_code_cmp in _carbon_pivot_cmp.columns:
                                            _carbon_limit_cmp = _carbon_pivot_cmp[_p_code_cmp].reindex(
                                                _years_crrem_cmp).astype(float)
                                    ...
                                    if not isinstance(_mixed_df_cmp, pd.DataFrame) or _mixed_df_cmp.empty:
                                        try:
                                            _active_sc_cmp = str(st.session_state.get("active_scenario", ""))
                                            _mixed_df_cmp = _mixed_use_records_to_df(
                                                (scenarios.get(_active_sc_cmp, {}) or {}).get("crrem_mixed_use", []))
                                        except Exception:
                                    ...
                                    if _components_cmp:
                                        ...
                                        _use_to_code_cmp = dict(zip(_pt_df_cmp["app_use"].astype(str),
                                                                    _pt_df_cmp["crrem_code"].astype(str)))
                                        ...
                                        for _u_cmp, _w_cmp in _components_cmp:
                                            ...
                                            if _code_cmp and _code_cmp in _carbon_pivot_cmp.columns and _tot_share_cmp > 0:
                                                _carbon_limit_cmp = _carbon_limit_cmp + (
                                                            float(_w_cmp) / float(_tot_share_cmp)) * _carbon_pivot_cmp[
                                                                        _code_cmp].reindex(_years_crrem_cmp).astype(
                                                    float)

                                if _carbon_limit_cmp is not None and not _carbon_limit_cmp.empty:
                                    _annual_crrem_limit_t_cmp = (_carbon_limit_cmp.astype(float) * float(
                                        _area)) / 1000.0
                    ...
                    for _idx_sc, _sc_name in enumerate(scenario_order):
                        ...
                        emissions_summary_rows.append({
                            "Scenario": _sc_name,
                            "Annual Net CO₂ first year (t/a)": float(_annual_emissions_series_t.iloc[0]) if len(
                                _annual_emissions_series_t) else 0.0,
                            "Annual Net CO₂ final year (t/a)": float(_annual_emissions_series_t.iloc[-1]) if len(
                                _annual_emissions_series_t) else 0.0,
                            "Cumulative Net CO₂ (t)": float(_cum_emissions_series_t.iloc[-1]) if len(
                                _cum_emissions_series_t) else 0.0,
                        })
                    ...
                    with lc1:
                        ...
                        if fig_energy_cost_annual_cmp.data:
                            ...
                            st_plotly_chart(fig_energy_cost_annual_cmp, use_container_width=True,
                                            key="scenario_energy_cost_annual_all")
                        else:
                    ...
                    with em1:
                        ...
                        if fig_emis_annual_cmp.data:
                            ...
                            st_plotly_chart(fig_emis_annual_cmp, use_container_width=True,
                                            key="scenario_emissions_annual_all")
                        else:
                    ...
                    with em2:
                        ...
                        if fig_emis_cum_cmp.data:
                            ...
                            st_plotly_chart(fig_emis_cum_cmp, use_container_width=True,
                                            key="scenario_emissions_cumulative_all")
                        else:
            ...
            with st.expander("Static Comparission", expanded=True):
                ...
                if not _area or _area <= 0:
                    ...
                    if not df_energy_src.empty:
                        ...
                        st_plotly_chart(fig_end_energy, use_container_width=True,
                                        key="scenario_end_energy_m2_by_source")
                    ...
                    if not df_energy_eu.empty:
                        ...
                        st_plotly_chart(fig_end_energy_eu, use_container_width=True,
                                        key="scenario_end_energy_m2_by_enduse")
...
def _loads_find_matching_load(reference_load: str, df_loads: pd.DataFrame) -> Optional[str]:
    """Return the best load column matching a reference load name.

    Matching uses the same prefix/suffix-tolerant normalization as the Load/Energy
    matching used by Annual System Efficiency. Exact normalized matches are preferred;
    a unique containment match is accepted only as a conservative fallback.
    """
    try:
        candidates = _loads_available_load_columns(df_loads)
        if not candidates:
            return None
        ...
        if len(reference_key) >= 4:
            partial = [
                c for c in candidates
                if (_loads_energy_match_key(c) in reference_key or reference_key in _loads_energy_match_key(c))
                   and len(_loads_energy_match_key(c)) >= 4
            ]
            if len(partial) == 1:
                return partial[0]
...
def _loads_build_scenario_comparison_df_uncached(
    ...
    """Build comparable Load KPIs for the same logical load across all scenarios.
    ...
    for sc_name in [str(x) for x in (scenario_names or []) if str(x).strip()]:
        ...
        try:
            df_sc_loads = get_loads_balance_df(file_bytes, filename, scenario_name=sc_name)
            matched_load = _loads_find_matching_load(reference_load, df_sc_loads)
        except Exception:
            df_sc_loads = pd.DataFrame()
            matched_load = None

        if matched_load and isinstance(df_sc_loads, pd.DataFrame) and matched_load in df_sc_loads.columns:
            try:
                load_series = pd.to_numeric(df_sc_loads[matched_load], errors="coerce").dropna()
                if not load_series.empty:
                    annual_load = float(load_series.sum())
                    peak_load = float(load_series.max())
                    p95_load = float(np.percentile(load_series.astype(float), 95))
            except Exception:
                pass

            try:
                if "doy" in df_sc_loads.columns:
                    daily_work = pd.DataFrame({
                        "doy": pd.to_numeric(df_sc_loads["doy"], errors="coerce"),
                        "_load": pd.to_numeric(df_sc_loads[matched_load], errors="coerce"),
                    }).dropna(subset=["doy", "_load"])
                    if not daily_work.empty:
                        daily_sums = daily_work.groupby("doy", as_index=False)["_load"].sum()
                        if not daily_sums.empty:
                            max_daily_load_sum = float(daily_sums["_load"].max())
            except Exception:
                max_daily_load_sum = 0.0
            ...
            if abs(float(annual_energy_for_coverage)) > 1e-12:
                annual_system_efficiency = float(annual_load) / float(annual_energy_for_coverage)
        ...
        rows.append({
            ...
            "Maximum Daily Load Sum (kWh/day)": float(max_daily_load_sum),
            "95th Percentile Load (kW)": float(p95_load),
            "Annual Energy for Load Coverage (kWh/a)": float(annual_energy_for_coverage),
        })
...
def _loads_pivot_hour_doy(df_in: pd.DataFrame, value_col: str) -> pd.DataFrame:
    ...
    if work.empty:
        return pd.DataFrame()
    return work.pivot_table(index="hour", columns="doy", values=value_col, aggfunc="mean").sort_index().sort_index(
        axis=1)


def _loads_ghost_trace_label(ghost_load: Optional[str], ghost_scenario: Optional[str] = None) -> str:
    """Return a compact, explicit legend label for ghost load overlays."""
    if not ghost_load:
        return "Ghost load"
    load_label = ui_name(ghost_load)
    scenario_label = str(ghost_scenario or "").strip()
    if scenario_label:
        return f"{load_label} — {scenario_label} (ghost)"
...
def _loads_add_ghost_contour(
        ...
        ghost_scenario: Optional[str] = None,
) -> go.Figure:
    """Overlay a gray contour representation of the ghost load on a heatmap-style figure."""
    if not ghost_load or df_loads is None or df_loads.empty or ghost_load not in df_loads.columns:
        return fig
    try:
        piv = _loads_pivot_hour_doy(df_loads, ghost_load)
        if piv.empty:
            return fig
        ghost_label = _loads_ghost_trace_label(ghost_load, ghost_scenario)
...
def _loads_typical_day_profile_figure(
        ...
        df_ghost_loads_in: Optional[pd.DataFrame] = None,
        ghost_scenario: Optional[str] = None,
) -> Tuple[go.Figure, int]:
    """Build the Typical Day by Month diagram.

    Each month is a compact 24-hour average profile positioned inside that month slot.
    The selected load comes from the active scenario dataframe. The optional ghost load
    can come from another scenario dataframe.
    """

    def _prepare_typical_source(df_source: pd.DataFrame, load_col: str) -> Tuple[pd.DataFrame, int]:
        if df_source is None or df_source.empty or not load_col or load_col not in df_source.columns:
        ...
        work[load_col] = pd.to_numeric(work[load_col], errors="coerce")
        work = work.dropna(subset=["_month_idx", "hour", load_col])
        work = work.loc[work["_month_idx"].between(1, 12) & work["hour"].between(0, 23)].copy()
        if work.empty:
            return pd.DataFrame(), 0
        group_cols = ["_month_idx", "_month_label", "hour"]
        typical_source = work.groupby(group_cols, as_index=False, observed=True)[load_col].mean()
        typical_source = typical_source.sort_values(["_month_idx", "hour"])
        return typical_source, int(len(work))

    typical, selected_row_count = _prepare_typical_source(df_loads_in, selected_load)
    ghost_typical = pd.DataFrame()
    ghost_label = _loads_ghost_trace_label(ghost_load, ghost_scenario) if ghost_load else ""
    if ghost_load:
        ghost_df = df_ghost_loads_in if isinstance(df_ghost_loads_in, pd.DataFrame) else df_loads_in
        ghost_typical, _ghost_row_count = _prepare_typical_source(ghost_df, ghost_load)
    ...
    def _profile_x(month_idx_series, hour_series):
        ...
        return pd.to_numeric(month_idx_series, errors="coerce") + (
                    (pd.to_numeric(hour_series, errors="coerce") - 11.5) / 34.0)
    ...
    if ghost_load and not ghost_typical.empty:
        plotted_ghost_months = 0
        for _, gdf in ghost_typical.groupby("_month_idx", sort=True):
            if gdf.empty:
                continue
            month_label = str(gdf["_month_label"].iloc[0])
            x_vals = _profile_x(gdf["_month_idx"], gdf["hour"])
            custom = np.stack([gdf["hour"].astype(float), np.repeat(month_label, len(gdf))], axis=-1)
            fig.add_trace(go.Scatter(
                x=x_vals,
                y=gdf[ghost_load].astype(float),
                mode="lines+markers",
                name=ghost_label,
                legendgroup=f"ghost_{ghost_scenario}_{ghost_load}",
                showlegend=(plotted_ghost_months == 0),
    ...
    return fig, int(selected_row_count)
...
with tab4:
    if uploaded_file:
        ...
        active_scenario_loads = str(
            st.session_state.get("active_scenario", globals().get("active_selected", "Base")) or "Base")
        df_loads = get_loads_balance_df(file_bytes_loads, uploaded_file.name, scenario_name=active_scenario_loads).copy(
            deep=False)
        ...
        st.metric("Active Scenario", active_scenario_loads)

        if not load_cols:
            st.warning(
                "No load columns found in the active scenario Loads_Balance sheet. Add at least one load column to use Loads Analysis.")
            st.stop()

        scenario_names_loads = [str(x) for x in st.session_state.get("scenarios", {}).keys() if str(x).strip()]
        if not scenario_names_loads:
            scenario_names_loads = [active_scenario_loads]
        if active_scenario_loads not in scenario_names_loads:
            scenario_names_loads.insert(0, active_scenario_loads)
        ...
        with load_select_col:
            ...
            if _previous_selected_load not in load_cols:
                ...
                _previous_match_key = _loads_energy_match_key(
                    _previous_selected_load) if _previous_selected_load else ""
                _matched_loads = [c for c in load_cols if
                                  _loads_energy_match_key(c) == _previous_match_key and _previous_match_key]
                st.session_state[_loads_selected_key] = _matched_loads[0] if _matched_loads else load_cols[0]
            selected_load = st.selectbox(
                "Select Load",
                load_cols,
                index=load_cols.index(st.session_state[_loads_selected_key]) if st.session_state.get(
                    _loads_selected_key) in load_cols else 0,
                format_func=ui_name,
        ...
        with ghost_scenario_col:
            ghost_scenario_option = st.selectbox(
                "Ghost scenario",
                ["None"] + scenario_names_loads,
                index=0,
                help="Optional scenario used only as a gray/dashed comparison overlay in Loads Analysis diagrams."
            )

        ghost_load = None
        ghost_scenario = None
        ghost_label = ""
        df_ghost_loads = pd.DataFrame()
        with ghost_load_col:
            if ghost_scenario_option != "None":
                ghost_scenario = str(ghost_scenario_option)
                df_ghost_loads = get_loads_balance_df(file_bytes_loads, uploaded_file.name,
                                                      scenario_name=ghost_scenario).copy(deep=False)
                ...
                if ghost_load_options:
                    ghost_load = st.selectbox(
                        "Ghost load",
                        ghost_load_options,
                        index=0,
                        format_func=ui_name,
                        help="Load from the selected ghost scenario shown in gray/dashed form for visual comparison."
                    )
                else:
                    st.selectbox("Ghost load", ["None"], index=0,
                                 help="No alternative load is available for the selected ghost scenario.")
            else:
                st.selectbox("Ghost load", ["None"], index=0, help="Select a ghost scenario first.")

        if ghost_load:
            for _c in ["doy", "hour"]:
                if _c in df_ghost_loads.columns:
                    df_ghost_loads[_c] = pd.to_numeric(df_ghost_loads[_c], errors="coerce")
            ghost_label = _loads_ghost_trace_label(ghost_load, ghost_scenario)
            st.caption(
                f"Ghost overlay active: {ghost_label} is shown in gray on Loads Analysis diagrams where the chart type supports an overlay."
            )
        ...
        load_heatmap = _loads_add_ghost_contour(load_heatmap, df_ghost_loads, ghost_load, ghost_scenario)
        ...
        monthly_total_load_bar.update_traces(marker_color=bar_color, name=ui_name(selected_load), showlegend=True,
                                             opacity=0.95)
        if ghost_load:
            ghost_totals_by_month = df_ghost_loads.groupby("month", as_index=False)[ghost_load].sum()
        ...
        with col3:
            ...
            st.metric(
                ...
                f"{annual_system_efficiency:,.2f}" if annual_system_efficiency is not None and np.isfinite(
        ...
        if ghost_load:
            try:
                df_ghost_bool = df_ghost_loads.copy()
                df_ghost_bool["ghost_exceed"] = (
                            pd.to_numeric(df_ghost_bool[ghost_load], errors="coerce") > thr).astype(int)
        ...
        profile_cols = ["hour", selected_load]
        ...
        ghost_day_profile = pd.DataFrame(columns=["hour", ghost_load] if ghost_load else ["hour"])
        if ghost_load and {"doy", "hour", ghost_load}.issubset(df_ghost_loads.columns):
            ghost_day_profile = (df_ghost_loads.loc[df_ghost_loads["doy"] == peak_doy, ["hour", ghost_load]]
                                 .copy())
            ghost_day_profile["hour"] = pd.to_numeric(ghost_day_profile["hour"], errors="coerce")
            ghost_day_profile[ghost_load] = pd.to_numeric(ghost_day_profile[ghost_load], errors="coerce")
            ghost_day_profile = ghost_day_profile.dropna(subset=["hour", ghost_load]).sort_values("hour")
        ...
        peak_day_fig.update_traces(line=dict(width=6, color=bar_color), marker=dict(size=12),
                                   name=ui_name(selected_load), showlegend=bool(ghost_load))
        ...
        try:
            ...
            _peak_day_hourly_hour_label = f"{int(_peak_day_hourly_hour)}" if float(
                _peak_day_hourly_hour).is_integer() else f"{_peak_day_hourly_hour:.1f}"
            st.caption(
        ...
        if threshold_analysis_enabled and threshold_analysis_kw is not None and isinstance(peak_day_threshold_stats,
                                                                                           dict):
            st.caption(
        ...
        peak_moment_source = df_loads.loc[:, [c for c in ["doy", "hour", "month", "day", selected_load] if
                                              c in df_loads.columns]].copy()
        ...
        if peak_moment_source.empty:
            ...
            peak_moment_day_profile[selected_load] = pd.to_numeric(peak_moment_day_profile[selected_load],
                                                                   errors="coerce")
            ...
            if ghost_load and {"doy", "hour", ghost_load}.issubset(df_ghost_loads.columns):
                ghost_peak_moment_day_profile = (
                    df_ghost_loads.loc[df_ghost_loads["doy"] == peak_moment_doy, ["hour", ghost_load]]
                    .copy())
                ghost_peak_moment_day_profile["hour"] = pd.to_numeric(ghost_peak_moment_day_profile["hour"],
                                                                      errors="coerce")
                ghost_peak_moment_day_profile[ghost_load] = pd.to_numeric(ghost_peak_moment_day_profile[ghost_load],
                                                                          errors="coerce")
                ghost_peak_moment_day_profile = ghost_peak_moment_day_profile.dropna(
                    subset=["hour", ghost_load]).sort_values("hour")
            ...
            peak_moment_day_fig.update_traces(line=dict(width=6, color=bar_color), marker=dict(size=12),
                                              name=ui_name(selected_load), showlegend=bool(ghost_load))
            ...
            _peak_hour_label = f"{int(peak_moment_hour)}" if float(
                peak_moment_hour).is_integer() else f"{peak_moment_hour:.1f}"
            st.caption(
            ...
            if threshold_analysis_enabled and threshold_analysis_kw is not None and isinstance(
                    peak_moment_threshold_stats, dict):
                st.caption(
                    f"Threshold analysis ({float(threshold_analysis_kw):,.1f} kW): "
        ...
        if exclude_weekends_typical and "weekday" not in df_loads.columns:
            st.warning(
                "Weekend exclusion requires a 'weekday' column in Loads_Balance. The typical-day diagram uses all days.")
        ...
        typical_day_fig, typical_day_row_count = _loads_typical_day_profile_figure(
            ...
            exclude_weekends=bool(exclude_weekends_typical),
            df_ghost_loads_in=df_ghost_loads,
        ...
        ldc_fig.update_traces(line=dict(width=6, color=bar_color), name=ui_name(selected_load),
                              showlegend=bool(ghost_load))
        r, g, b = pcolors.hex_to_rgb(bar_color)
        ldc_fig.update_traces(fill="tozeroy", fillcolor=f"rgba({r},{g},{b},0.25)")
        if ghost_load:
            ghost_ldc_vals = pd.to_numeric(df_ghost_loads[ghost_load], errors="coerce").dropna()
            ghost_ldc_sorted = ghost_ldc_vals.sort_values(ascending=False).reset_index(drop=True)
            ghost_ldc_pct = (np.arange(1, len(ghost_ldc_sorted) + 1) / len(ghost_ldc_sorted)) * 100 if len(
                ghost_ldc_sorted) else []
        ...
        with st.expander("Scenario Comparisson", expanded=False):
            ...
            if not _load_scenario_comparison_enabled:
                st.caption(
                    "Scenario comparison is disabled. Enable it to calculate and display the four all-scenario load KPI charts.")
            else:
                _load_cmp_scenarios = [
                ...
                if _load_cmp_df.empty:
                    ...
                    for _i_sc, _sc in enumerate(_load_cmp_scenarios):
                        ...
                        if _hit.empty:
                            continue
                        _row = _hit.iloc[0]
                        _col = _load_cmp_colors.get(str(_sc),
                                                    SCENARIO_COLOR_PALETTE[_i_sc % len(SCENARIO_COLOR_PALETTE)])
                    ...
                    with _cmp_row1_col1:
                        st_plotly_chart(_fig_load_cmp_annual, use_container_width=True,
                                        key="loads_scenario_comparison_annual")
                    with _cmp_row1_col2:
                        st_plotly_chart(_fig_load_cmp_power, use_container_width=True,
                                        key="loads_scenario_comparison_power")

                    _cmp_row2_col1, _cmp_row2_col2 = st.columns(2)
                    with _cmp_row2_col1:
                        st_plotly_chart(_fig_load_cmp_eff, use_container_width=True,
                                        key="loads_scenario_comparison_efficiency")
                    with _cmp_row2_col2:
                        st_plotly_chart(_fig_load_cmp_daily, use_container_width=True,
                                        key="loads_scenario_comparison_daily")
...
with tab8:
    if (not IS_VIEWER_MODE) and uploaded_file:
        ...
        if _RAW_ENERGY_DRAFT_KEY not in st.session_state:
            st.session_state[_RAW_ENERGY_DRAFT_KEY] = get_global_energy_balance_df(file_bytes, uploaded_file.name).copy(
                deep=True)
        ...
        with st.expander("Energy_Balance (monthly, kWh)", expanded=True):
            ...
            if use_scenario_energy_scope:
                ...
                st.caption(
                    "Editing scope: **global/base Energy_Balance**. Scenario-specific overrides, if any, are not changed.")
            ...
            if st.session_state.get(energy_flash_key) == "updated":
                ...
                st.info(
                    "Energy_Balance columns renamed in draft. Click **Update Data** to apply to the selected scope.")
                del st.session_state[energy_flash_key]
            elif st.session_state.get(energy_flash_key) == "removed_override":
                st.info(
                    f"Scenario-specific Energy_Balance override removed for '{active_raw_scenario}'. This scenario now uses the global/base data.")
                del st.session_state[energy_flash_key]
            ...
            def _get_energy_draft_for_scope() -> pd.DataFrame:
                if use_scenario_energy_scope:
                    drafts = _scenario_energy_drafts()
                    if active_raw_scenario not in drafts or not isinstance(drafts.get(active_raw_scenario),
                                                                           pd.DataFrame):
                ...
                return sanitize_energy_balance_df(st.session_state.get(_RAW_ENERGY_DRAFT_KEY, pd.DataFrame())).copy(
                    deep=True)
            ...
            with c1:
                new_col_name = st.text_input("Add new End Use column", value="",
                                             key=f"raw_add_energy_col_{wb_hash}_{scope_suffix}")
            with c2:
                new_col_default = numeric_input("Default value", 0.0,
                                                key=f"raw_add_energy_default_{wb_hash}_{scope_suffix}", fmt="{:.3f}")
            with c3:
                if st.button("Add column", key=f"raw_add_energy_btn_{wb_hash}_{scope_suffix}",
                             use_container_width=True):
                    ...
                    if apply_rename_energy:
                        ...
                        if len(set(final_cols)) != len(final_cols):
                            ...
                            try:
                                ...
                                onsite_lst = [ONSITE_GENERATION_ENDUSE if str(x) == LEGACY_PV_ENDUSE else str(x) for x
                                              in onsite_lst]
        ...
        with st.expander("Loads_Balance (hourly, kW)", expanded=False):
            ...
            if use_scenario_loads_scope:
                ...
                st.caption(
                    "Editing scope: **global/base Loads_Balance**. Scenario-specific overrides, if any, are not changed.")
            ...
            if st.session_state.get(loads_flash_key) == "updated":
                ...
                st.info(
                    f"Scenario-specific Loads_Balance override removed for '{active_raw_scenario}'. This scenario now uses the global/base data.")
                del st.session_state[loads_flash_key]
            ...
            def _get_loads_draft_for_scope() -> pd.DataFrame:
                ...
                return sanitize_loads_balance_df(st.session_state.get(_RAW_LOADS_DRAFT_KEY, pd.DataFrame())).copy(
                    deep=True)
            ...
            with c2:
                new_load_default = numeric_input("Default value", 0.0,
                                                 key=f"raw_add_load_default_{wb_hash}_{loads_scope_suffix}",
                                                 fmt="{:.3f}")
            with c3:
                if st.button("Add column", key=f"raw_add_load_btn_{wb_hash}_{loads_scope_suffix}",
                             use_container_width=True):
                    ...
                    if apply_rename_loads:
                        ...
                        for _, r in edited_rename_df.iterrows():
                            ...
                            if new in fixed_in_df:
                                st.error(
                                    f"'{new}' is reserved for time/meta columns and cannot be used as a load name.")
                                mapping = None
                                break
                        ...
                        if mapping is not None:
                            if len(set(final_cols)) != len(final_cols):
                                ...
                                try:
                                    ...
                                    if not isinstance(onsite_lst, list) or len(onsite_lst) == 0:
                                        onsite_lst = [ONSITE_GENERATION_ENDUSE]
                                    onsite_lst = [ONSITE_GENERATION_ENDUSE if str(x) == LEGACY_PV_ENDUSE else str(x) for
                                                  x in onsite_lst]
                                    updated_lst = [mapping.get(x, x) for x in onsite_lst]
                                    uniq = []
                                    for x in updated_lst:
